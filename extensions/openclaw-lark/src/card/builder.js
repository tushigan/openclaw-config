...
function buildStageProgressBar(current, total) {
    const safeCurrent = Math.max(1, Math.min(current, total));
    const filled = '#'.repeat(safeCurrent);
    const empty = '-'.repeat(Math.max(0, total - safeCurrent));
    return `[${filled}${empty}]`;
}
function resolveStageProgress(params) {
    ...
    if (isComplete) {
        return {
            current: 4,
            total,
            zh: isError ? '执行失败' : isAborted ? '已中断' : '已完成',
            en: isError ? 'Failed' : isAborted ? 'Stopped' : 'Completed',
        };
    }
    if (hasPartialText) {
        return {
...
function buildStageProgressElement(params) {
    const progress = resolveStageProgress(params);
    const percent = Math.round(progress.current / progress.total * 100);
    const bar = buildStageProgressBar(progress.current, progress.total);
    return {
        tag: 'markdown',
        content: `**Stage progress** ${bar} ${percent}%\n${progress.en}`,
        i18n_content: {
            zh_cn: `**阶段进度** ${bar} ${percent}%\n${progress.zh}`,
            en_us: `**Stage progress** ${bar} ${percent}%\n${progress.en}`,
        },
        text_size: 'notation',
    };
}
...
function buildRuntimeIdentityLabel(metrics) {
    if (!metrics)
        return undefined;
    const parts = [];
    const agentId = typeof metrics.agentId === 'string' ? metrics.agentId.trim() : '';
...
function formatFooterRuntimeSegments(params) {
    ...
    const detailZh = [];
    const detailEn = [];
    // --- Primary line: status and elapsed ---
    if (footer?.status) {
        if (isError) {
            primaryZh.push('出错');
    ...
    if (footer?.model) {
        const runtimeLabel = buildRuntimeIdentityLabel(metrics);
        if (runtimeLabel) {
            detailZh.push(runtimeLabel);
            detailEn.push(runtimeLabel);
        }
    }
...
function buildStreamingPreAnswerCard(params) {
    ...
    const progress = resolveStageProgress({ hasToolUse: hasSteps });
    const progressPercent = Math.round(progress.current / progress.total * 100);
    const progressBar = buildStageProgressBar(progress.current, progress.total);
    const elements = [];
    if (showToolUse) {
        elements.push(hasSteps ? buildStreamingToolUseActivePanel({ steps: steps, elapsedMs }) : buildStreamingToolUsePendingPanel());
    }
    elements.push(buildStageProgressElement({ hasToolUse: hasSteps }));
    elements.push({
        tag: 'markdown',
        content: '',
        text_align: 'left',
    ...
    return {
        ...
        config: {
            ...
            locales: ['zh_cn', 'en_us'],
            summary: {
                content: `${progress.en} ${progressBar} ${progressPercent}%`,
                i18n_content: {
                    zh_cn: `${progress.zh} ${progressBar} ${progressPercent}%`,
                    en_us: `${progress.en} ${progressBar} ${progressPercent}%`,
                },
            },
        },
        body: { elements },
