...
alias run-help=man
alias skillhub='skillhub --dir ~/.openclaw/workspace/skills'
...
_openclaw_channels_list () {
	_arguments -C "--all[Include bundled and installable catalog channels]" "--json[Output JSON]"
}
...
_openclaw_daemon_restart () {
	_arguments -C "--force[Restart immediately without waiting for active gateway work]" "--safe[Request an OpenClaw-aware restart after active work drains]" "--wait[Wait duration before forcing restart (ms, 10s, 5m; 0 waits indefinitely)]" "--json[Output JSON]"
}
...
_openclaw_feishu_diagnose () {
	_arguments -C "--trace[按 message_id 追踪完整处理链路]" "--analyze[分析追踪日志（需配合 --trace 使用）]"
}
_openclaw_gateway () {
	local -a commands
	local -a options
...
_openclaw_models_auth_list () {
	_arguments -C "--provider[Filter by provider id]" "--agent[Agent id (default: configured default agent)]" "--json[Output JSON]"
}
...
_openclaw_proxy_validate () {
	_arguments -C "--json[Print machine-readable JSON]" "--proxy-url[Proxy URL to validate instead of config/env]" "--allowed-url[Destination expected to succeed through the proxy]" "--denied-url[Destination expected to be blocked by the proxy]" "--apns-reachable[Also verify sandbox APNs HTTP/2 is reachable through the proxy]" "--apns-authority[APNs authority to probe with --apns-reachable]" "--timeout-ms[Per-request timeout in milliseconds]"
}
_openclaw_qr () {
	_arguments -C "--remote[Use gateway.remote.url and gateway.remote token/password (ignores device-pair publicUrl)]" "--url[Override gateway URL used in the setup payload]" "--public-url[Override gateway public URL used in the setup payload]" "--token[Override gateway token for setup payload]" "--password[Override gateway password for setup payload]" "--setup-code-only[Print only the setup code]" "--no-ascii[Skip ASCII QR rendering]" "--json[Output JSON]"
}
...
_openclaw_sessions () {
	local -a commands
	local -a options
	_arguments -C "--json[Output as JSON]" "--verbose[Verbose logging]" "--store[Path to session store (default: resolved from config)]" "--agent[Agent id to inspect (default: configured default agent)]" "--all-agents[Aggregate sessions across all configured agents]" "--active[Only show sessions updated within the past N minutes]" "--limit[Max sessions to show (default: 100; use \"all\" for full output)]" "1: :_values 'command' 'cleanup[Run session-store maintenance now]' 'export-trajectory[Export a redacted trajectory bundle for a stored session]'" "*::arg:->args"
	case $state in
		(args) case $line[1] in
...
export PATH='/Users/a123/.local/bin:/opt/homebrew/opt/node@22/bin:/opt/homebrew/Cellar/node@22/22.22.2_2/bin:/usr/bin:/bin:/opt/homebrew/opt/node@22/bin:/opt/homebrew/bin:/opt/homebrew/sbin:/usr/local/bin:/usr/sbin:/sbin:/Users/a123/.local/bin'