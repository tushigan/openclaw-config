...
# DELIVERY_DIR 不再是全局常量，改为动态函数 get_delivery_dir_for_project()
...
def get_delivery_dir_for_project(project_dir: Path) -> Path:
    """
    根据项目目录动态确定交付目录。
    ...
    Returns:
        对应 workspace 的 feishu-deliver 目录路径
    """
    workspace = detect_workspace_from_project(project_dir)
    return get_delivery_dir(workspace)
...
def make_preview(source_path: Path, project_id: str, delivery_dir: Path) -> tuple[Path, int]:
...
def copy_original(source_path: Path, project_id: str, delivery_dir: Path) -> tuple[Path, int]:
...
def make_zip(source_path: Path, project_id: str, delivery_dir: Path) -> tuple[Path, int]:
...
def main() -> int:
    ...
    # 动态确定交付目录（根据项目所在的 workspace）
    delivery_dir = ensure_delivery_dir(project_dir)
    ...
    if original_size > FEISHU_LIMIT_BYTES:
        preview_path, preview_size = make_preview(image_path, project_id, delivery_dir)
        original_copy_path, original_copy_size = copy_original(image_path, project_id, delivery_dir)
        zip_path, zip_size = make_zip(image_path, project_id, delivery_dir)
    else:
        original_copy_path, original_copy_size = copy_original(image_path, project_id, delivery_dir)
