...
# DISTILLER_ROOT 不再是全局常量，改为从项目路径动态推断
# layout_analyzer 的导入延迟到 main() 函数中，因为需要先确定 workspace

def get_distiller_scripts_dir(workspace: str) -> Path:
    """获取 distiller scripts 目录"""
    return get_distiller_root(workspace) / "scripts"
...
def resolve_skeleton_reference(distill, distill_path, distiller_root, _export_skeleton_png_func):
    """
    解析蒸馏卡中的骨架图路径，优先 PNG，回退 SVG。
    如果 SVG 更新，自动重新生成 PNG。

    Args:
        distill: 蒸馏卡数据
        distill_path: 蒸馏卡路径
        distiller_root: distiller 根目录（从 workspace 推断）
        _export_skeleton_png_func: layout_analyzer._export_skeleton_png 函数
    """
    if not distill:
        return ""
    ...
    if svg_exists:
        ...
        if png_stale:
            ...
            regenerated = _export_skeleton_png_func(svg_rel, distill_id=distill_id)
            if regenerated:
...
def main():
    ...
    _export_skeleton_png = None

    output_path = Path(args.output).resolve()
    project_dir = output_path.parent

    # 动态检测 workspace 并设置 distiller 路径
    workspace = detect_workspace_from_project(project_dir)
    distiller_root = get_distiller_root(workspace)
    distiller_scripts_dir = get_distiller_scripts_dir(workspace)

    # 动态导入 layout_analyzer（需要从对应 workspace 的 distiller 加载）
    ...
    if distill:
        skeleton_path = resolve_skeleton_reference(distill, args.distill, distiller_root, _export_skeleton_png)
        if skeleton_path:
