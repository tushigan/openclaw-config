...
from workspace_utils import detect_workspace_from_project, get_distiller_cards_dir, get_distiller_root

# 不再使用全局常量，改为从项目路径动态推断
...
def resolve_card_path(distill_id: str, workspace: str) -> Path:
    """根据 workspace 解析蒸馏卡路径"""
    cards_dir = get_distiller_cards_dir(workspace)
    return cards_dir / f'{distill_id}.json'


def resolve_asset(raw_path: str, card_path: Path, distiller_root: Path) -> Path | None:
    if not raw_path:
        return None
...
def copy_skeleton(distill_data: dict, card_path: Path, project_dir: Path, distiller_root: Path) -> tuple[str, str]:
    ...
    source_path = resolve_asset(skeleton_png, card_path, distiller_root) if skeleton_png else None
    used_field = 'skeleton_png'
    if source_path is None and skeleton_svg:
        source_path = resolve_asset(skeleton_svg, card_path, distiller_root)
...
def main() -> int:
    ...
    card_path = resolve_card_path(distill_id, workspace)
    ...
    skeleton_path, skeleton_source_field = copy_skeleton(distill_data, card_path, project_dir, distiller_root)
    layout = distill_data.get('layout_analysis', {}) or {}
