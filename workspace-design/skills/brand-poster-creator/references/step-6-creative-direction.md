...
⚠️ **关键**：艾特标签必须在代码块外，表格放在 plaintext 代码块内（与文案审批格式一致）。

🔴 **强制格式**（完全复制文案审批的成功格式）：

```markdown
{使用脚本返回的 .message 字段内容}
...
🔴 **关键要点**：
1. **艾特标签在正常文本中**（不在代码块内）
2. **表格在 plaintext 代码块内**（用 \`\`\`plaintext 包裹）
3. **与文案审批格式完全一致**（文案审批的艾特成功就是这个格式）
4. **避免消息被飞书转换成卡片**（plaintext 代码块防止自动渲染）

🔴 **严格禁止**：
- ❌ 表格不在 plaintext 代码块内（会被飞书渲染成卡片）
- ❌ 艾特标签在代码块内（代码块内的艾特不生效）
- ❌ 使用 Markdown 表格而不是 plaintext 代码块

**为什么这很重要**：
- 文案审批成功：艾特在外 + 表格在 plaintext 代码块内
- 创意方向失败：整个消息被飞书渲染成卡片，艾特不生效
- **plaintext 代码块防止消息被自动转换成卡片**
