...
⚠️ **关键**：必须关闭流式输出，使用普通消息格式，才能正确@人。

🔴 **发送审批消息的特殊要求**：

由于飞书的流式输出使用交互卡片，卡片内的@标签不会生效，因此**必须使用普通消息（非流式）来发送审批请求**。

**步骤**：
1. 在回复消息前，明确说明"我将发送审批请求（非流式输出）"
2. 使用普通文本回复（不使用流式卡片）
3. 格式与文案审批完全一致
...
1. **使用普通消息**（不使用流式输出卡片）
2. **艾特标签在正常文本中**（不在代码块内）
3. **表格在 plaintext 代码块内**（防止被渲染成卡片）
4. **与文案审批格式完全一致**

🔴 **严格禁止**：
- ❌ 使用流式输出发送审批消息（卡片内@不生效）
- ❌ 使用 Markdown 表格（会被转换成卡片）
- ❌ 艾特标签在代码块内
