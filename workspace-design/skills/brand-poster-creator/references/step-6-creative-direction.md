...
⚠️ **双重保险策略**：为确保艾特功能100%成功，采用分段发送策略。

🔴 **第一条消息**（艾特请求 - 极简格式）：

```markdown
<at user_id="ou_xxx">肖宁劼</at> <at user_id="ou_yyy">林育丰</at> <at user_id="ou_zzz">涂是淦</at> 创意方向已生成，请审核确认（详情见下一条消息）
```
...
1. **最高成功率**：艾特消息极简，飞书100%能解析
2. **文案审批的本质**：虽然文案审批看起来是一条消息，但艾特和内容是分离的逻辑
3. **双重保险**：即使第一条失败，第二条仍然展示完整信息
4. **已验证有效**：分段发送在其他场景100%成功

🔴 **实施要点**：
- 两条消息之间不要有任何延迟，立即连续发送
- 第一条消息只有1行
- 第二条消息包含所有详细信息
