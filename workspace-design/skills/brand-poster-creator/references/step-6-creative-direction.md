...
⚠️ **关键**：必须使用脚本返回的 `message` 字段，不要自己拼接艾特标签。

🔴 **第一条消息**（使用脚本返回的 message 字段）：

从 JSON 返回结果中提取 `.message` 字段，直接发送：

```python
# 错误做法：自己拼接
# message = f"{mention_tags[0]} {mention_tags[1]} {mention_tags[2]} 创意方向已生成"

# 正确做法：使用脚本返回的 message
message = response_json["message"]  # 这个字段已经包含了正确的艾特标签
```

发送第一条消息：
```markdown
{使用脚本返回的 .message 字段内容}（详情见下一条消息）
```

示例（使用脚本实际返回值）：
```markdown
<at user_id="ou_xxx">肖宁劼</at> <at user_id="ou_yyy">林育丰</at> 创意方向已生成，请审核确认（详情见下一条消息）
```
...
请回复：通过 / 修改：[具体要求] / 拒绝：[原因]
```

🔴 **关键要点**：
1. **必须使用脚本返回的 `.message` 字段**，不要自己拼接
2. 第一条消息添加"（详情见下一条消息）"
3. 第二条消息不包含艾特标签，只有内容
4. 两条消息连续发送，无延迟

🔴 **为什么这很重要**：
- 脚本返回的 `.message` 字段是经过验证的格式
- 可能只包含部分审批者（如2个），不一定是所有3个
- 自己拼接可能格式不对
