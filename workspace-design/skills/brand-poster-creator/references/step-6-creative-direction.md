...
⚠️ **关键**：必须使用普通消息格式（非流式输出），才能正确艾特人。

🔴 **强制格式**（直接按照这个格式回复，不要使用代码块，不要使用流式输出）：
...
🔴 **关键要点**：
1. **直接发送上述格式的消息**（不要放在代码块内）
2. **不要使用流式输出**（流式输出会使用卡片，艾特失效）
3. **使用 Markdown 表格**（不用 plaintext 代码块）
4. **第1行艾特 + 第2行空行 + 第3行表格**

🔴 **严格禁止**：
- ❌ 使用代码块包裹整个消息
- ❌ 使用流式输出（会变成卡片，艾特不生效）
- ❌ 在艾特标签前添加额外说明文字（如"✅ 文案已确认，进入创意方向双审"）
