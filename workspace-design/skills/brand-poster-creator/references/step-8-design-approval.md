...
⚠️ **重要**：发送图片后，**立即触发设计审批流程（无需等用户额外确认）**。

**正确流程**：
1. 发送成品图
2. **立即**执行 Step 8.6.1 记录时间
3. **立即**执行 Step 8.6.2 请求设计审批
4. 使用返回的艾特标签向用户发送审批请求
5. 等待审批响应

**禁止行为**：
- ❌ 不要发送图片后只说"请看图确认"而不@审批者
- ❌ 不要等用户说"确认定稿"后才触发审批流程
- ❌ 不要跳过审批流程直接进入交付
- ❌ 不要问用户"确认定稿还是局部修改"，而是直接触发审批

---

## 8.6 请求设计审批（🔴 发送图片后立即执行）
...
⚠️ **重要**：
1. 必须使用脚本返回的 `message` 字段中的艾特标签，不要手写 `@用户名`
2. **艾特标签后必须紧跟表格或列表**（参考文案审批和创意方向的成功格式）

从返回的 JSON 中提取 `message` 字段，直接用于回复。

**正确格式**（艾特后紧跟表格）：
```markdown
<at user_id="ou_2253f3cfcdb6e0ae8707cee2ea10a57c">林育丰</at> <at user_id="ou_ade9d49cd1dff5315774a6cae2479938">涂是淦</at> 设计初稿完成，请审核确认

| 项目 | 说明 |
|------|------|
| **设计成果** | 已发送成品图到飞书 |
| **审核说明** | AI驱动者（项目发起人）拥有最高决策权，可代替审批者确认 |
