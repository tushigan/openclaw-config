...
DEFAULT_WORKFLOW_HOME = Path(
    os.environ.get(
        "PRODUCT_PHOTO_WORKFLOW_HOME",
        Path.home() / "Documents" / "Codex" / "product-photography-workflow",
    )
).expanduser().resolve()
DEFAULT_OUTPUT_ROOT = Path(
    os.environ.get(
        "PRODUCT_PHOTO_OUTPUT_ROOT",
        DEFAULT_WORKFLOW_HOME / "outputs",
    )
).expanduser().resolve()
...
TASK_CAMERA_PREVIEW_FILE = "camera_preview_manifest.json"
...
TASK_ROLE_ORDER = [
    "page_draft",
    "camera_storyboard_reference",
    "composition_reference",
    "scene_reference",
    "style_reference",
    "lighting_reference",
    "product_state_reference",
    "package_material_reference",
    "texture_identity_reference",
    "other_reference",
]
...
def build_camera_preview_id(index: int) -> str:
...
def ensure_task_tree(task_dir: Path) -> None:
    for rel in [
        ...
        "references/camera_storyboard_reference",
        ...
        "references/package_material_reference",
        "references/texture_identity_reference",
...
def blank_task_brief(project_id: str, task_id: str, task_name: str, task_type: str, output_ratio: str, goal: str) -> dict[str, Any]:
    return {
        ...
        "must_avoid": [],
        "references_summary": {},
        "reference_roles_present": [],
        "reference_paths": {role: [] for role in TASK_ROLE_ORDER},
        "camera_preview_required": False,
        "camera_plan": {
            "hero_shot_intent": "",
            "shot_priority": [],
            "camera_angle_lock": "",
...
def blank_task_camera_preview_manifest(project_id: str, task_id: str) -> dict[str, Any]:
    return {
        "version": "1.0",
        "project_id": project_id,
        "task_id": task_id,
...
def blank_task_state(
    ...
    return {
        ...
        "selected_camera_preview_id": "",
        "latest_camera_preview_id": "",
        "latest_camera_preview_output": "",
        "latest_output": "",
        "workflow_flags": {
...
def initialize_task_files(
    ...
    camera_preview_manifest = blank_task_camera_preview_manifest(state.get("project_id", ""), task_id)
    ...
    write_json(task_dir / TASK_CAMERA_PREVIEW_FILE, camera_preview_manifest)
...
def next_camera_preview_id(task_dir: Path) -> str:
    preview_manifest = read_json(task_dir / TASK_CAMERA_PREVIEW_FILE, {})
    existing = preview_manifest.get("previews", [])
    return build_camera_preview_id(len(existing) + 1)
