...
TASK_KINDS = {"task_brief", "task_camera_preview", "task_generation_review"}
...
def main() -> None:
    ...
    parser.add_argument(
        "--kind",
        required=True,
        choices=["risk_continue", "creative_direction", "generation_review", "task_brief", "task_camera_preview", "task_generation_review"],
        help="确认类型",
    )
    ...
    if args.kind in TASK_KINDS:
        ...
        if args.kind == "task_brief":
            ...
            camera_preview_required = bool(brief.get("camera_preview_required", False))
            brief["confirmation"] = {
                "recorded_at": utc_now(),
            ...
            update_task_state(
                task_dir,
                current_stage=(
                    "task_camera_preview_ready"
                    if approved and camera_preview_required
                    else "task_brief_confirmed" if approved else "task_brief_waiting_confirm"
                ),
                ...
                workflow_flag_updates={
                    ...
                    "generation_allowed": approved and not camera_preview_required,
                },
            )
            update_task_manifest_entry(
                project_dir,
                ...
                {
                    "status": (
                        "camera_preview_required"
                        if approved and camera_preview_required
                        else "brief_confirmed" if approved else "brief_revision_requested"
                    ),
                },
            )
            update_state(
        ...
        elif args.kind == "task_camera_preview":
            preview_path = task_dir / TASK_CAMERA_PREVIEW_FILE
            preview_manifest = read_json(preview_path, {})
            ...
            preview_manifest["review"] = review
            preview_manifest["latest_status"] = "approved" if approved else "revision_requested"
            if args.selected_output:
                preview_manifest["latest_output"] = args.selected_output
            for item in preview_manifest.get("previews", []):
                if item.get("preview_id") == preview_id:
                    item["status"] = preview_manifest["latest_status"]
                    if args.selected_output:
                        item["output"] = args.selected_output
                    item["review"] = review
            write_json(preview_path, preview_manifest)

            latest_preview_output = args.selected_output or task_state.get("latest_camera_preview_output", "")
            update_task_state(
                task_dir,
                current_stage="task_camera_preview_confirmed" if approved else "task_camera_preview_waiting_confirm",
                stage_status="ready" if approved else "waiting_user",
                workflow_flag_updates={
                    "camera_preview_confirmed": approved,
                    "generation_allowed": approved,
                },
                field_updates={
                    "selected_camera_preview_id": preview_id,
                    "latest_camera_preview_id": preview_id,
                    "latest_camera_preview_output": latest_preview_output,
                },
            )
            update_task_manifest_entry(
                project_dir,
                task_state["task_id"],
                {
                    "status": "camera_preview_confirmed" if approved else "camera_preview_revision_requested",
                    "selected_camera_preview_id": preview_id,
                    "latest_camera_preview_id": preview_id,
                    "latest_camera_preview_output": latest_preview_output,
                },
