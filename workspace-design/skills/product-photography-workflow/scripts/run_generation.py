...
def run_task_generation(args: argparse.Namespace, project_dir: Path) -> int:
    ...
    if not args.force and not task_state.get("workflow_flags", {}).get("generation_allowed", False):
        if task_brief.get("camera_preview_required", False):
            raise SystemExit("机位定稿图尚未确认，不能进入最终生图。可先执行 build_camera_preview_prompt.py、run_camera_preview.py 和 record_confirmation.py --kind task_camera_preview。")
    ...
    package_material_refs = task_asset_entries_by_role(task_assets, ["package_material_reference"])
    texture_identity_refs = task_asset_entries_by_role(task_assets, ["texture_identity_reference"])
    product_refs = dedupe_entries(
        base_product_refs + package_material_refs + texture_identity_refs + product_state_refs,
        args.max_product_refs,
    )
    ...
    task_round_payload = {
        ...
        "camera_preview_id": confirmed_preview_id if layout_authority_role == "confirmed_camera_preview" else "",
        "camera_preview_refs": [item["absolute_path"] for item in camera_preview_refs],
        "camera_storyboard_refs": [item["absolute_path"] for item in camera_storyboard_refs],
        "package_material_refs": [item["absolute_path"] for item in package_material_refs],
        "texture_identity_refs": [item["absolute_path"] for item in texture_identity_refs],
        "product_state_refs": [item["absolute_path"] for item in product_state_refs],
        "style_refs": [item["absolute_path"] for item in style_refs],
        "background_refs": [item["absolute_path"] for item in background_refs],
