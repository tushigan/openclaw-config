...
require("./ssrf-runtime-BFVSngaE.js");
var _defaultsB6UFvvXy = require("./defaults-B6UFvvXy.js");
...
function resolveOllamaApiBase(configuredBaseUrl) {
  if (!configuredBaseUrl) return _defaultsB6UFvvXy.n;
...
async function queryOllamaModelShowInfo(apiBase, modelName) {
  ...
  try {
    const { response, release } = await (0, _fetchGuardDFNmfAZx.n)({
...
function buildOllamaModelDefinition(modelId, contextWindow, capabilities) {
  ...
  return {
    ...
    reasoning,
    input,
    cost: _defaultsB6UFvvXy.i,
    contextWindow: contextWindow ?? 128e3,
    maxTokens: _defaultsB6UFvvXy.a,
    compat
  };
}
