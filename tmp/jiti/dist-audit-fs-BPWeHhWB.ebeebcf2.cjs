...
const log = (0, _subsystemCxWoQXRD.t)("security/windows-acl");
...
function resolveWindowsSystemCommand(command, env) {
  const root = (0, _windowsInstallRoots2thIF_8W.t)(env ?? process.env).systemRoot;
...
async function inspectWindowsAcl(targetPath, opts) {
  const exec = opts?.exec ?? _execKfr6njO_.i;
  try {
