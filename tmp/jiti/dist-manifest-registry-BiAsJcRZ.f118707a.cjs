...
var _installPathsBj7Ll1xM = require("./install-paths-Bj7Ll1xM.js");
var _minHostVersionD6GrnMo = require("./min-host-version-D6GrnMo-.js");
...
function resolveRecoveredManagedNpmPluginId(params) {
  ...
  return (0, _installPathsBj7Ll1xM.c)(pluginId) ? void 0 : pluginId;
}
function buildRecoveredManagedNpmInstallRecords(options = {}) {
  const npmRoot = options.stateDir ? _nodePath.default.join(options.stateDir, "npm") : (0, _installPathsBj7Ll1xM.a)(options.env);
...
async function readPersistedInstalledPluginIndexInstallRecords(options = {}) {
  return extractPluginInstallRecordsFromPersistedInstalledPluginIndex(await (0, _jsonFilesDPM4MwsB.i)(resolveInstalledPluginIndexStorePath(options)));
}
function readPersistedInstalledPluginIndexInstallRecordsSync(options = {}) {
  return extractPluginInstallRecordsFromPersistedInstalledPluginIndex((0, _jsonFilesDPM4MwsB.a)(resolveInstalledPluginIndexStorePath(options)));
}
...
function normalizePluginsConfigWithResolver(config, normalizePluginId = _configNormalizationSharedDamsNnJD.n) {
  return (0, _configNormalizationSharedDamsNnJD.i)(config, normalizePluginId);
}
...
const hasExplicitPluginConfig = exports.n = _configNormalizationSharedDamsNnJD.t;
const isBundledChannelEnabledByChannelConfig = _configNormalizationSharedDamsNnJD.r;
...
function normalizePreferredPluginIds(raw) {
  return (0, _stringNormalizationC5SGsaST.a)(raw);
}
...
function mergePackageChannelMetaIntoChannelConfigs(params) {
  ...
  if (!channelId || (0, _prototypeKeysBWjW0VW.t)(channelId) || !params.channelConfigs || !Object.prototype.hasOwnProperty.call(params.channelConfigs, channelId)) return params.channelConfigs;
  ...
  for (const [key, value] of Object.entries(params.channelConfigs)) if (!(0, _prototypeKeysBWjW0VW.t)(key)) merged[key] = value;
  merged[channelId] = {
    ...existing,
    ...(label ? { label } : {}),
...
function mergeCatalogChannelConfigs(params) {
  ...
  const merged = Object.create(null);
  for (const [key, value] of Object.entries(params.catalogChannelConfigs)) if (!(0, _prototypeKeysBWjW0VW.t)(key)) merged[key] = value;
  for (const [key, value] of Object.entries(params.manifestChannelConfigs ?? {})) if (!(0, _prototypeKeysBWjW0VW.t)(key)) {
...
function buildRecord(params) {
  ...
  const officialCatalogManifest = params.candidate.origin !== "bundled" ? (0, _officialExternalPluginCatalog64MlR6o.r)((0, _officialExternalPluginCatalog64MlR6o.n)(params.candidate.packageName) ?? {}) : void 0;
...
function matchesInstalledPluginRecord(params) {
  ...
  const candidateSource = (0, _utilsD5swhEXt.p)(params.candidate.source, params.env);
  const trackedPaths = [record.installPath, record.sourcePath].filter((entry) => typeof entry === "string" && entry.trim().length > 0).map((entry) => (0, _utilsD5swhEXt.p)(entry, params.env));
  if (trackedPaths.length === 0) return false;
  return trackedPaths.some((trackedPath) => {
    return candidateSource === trackedPath || (0, _discoveryCVL9KJt.d)(trackedPath, candidateSource);
  });
}
...
function isTrustedOfficialPluginInstall(params) {
  ...
  const catalogEntry = (0, _officialExternalPluginCatalog64MlR6o.n)(packageName);
  if (!catalogEntry || (0, _officialExternalPluginCatalog64MlR6o.s)(catalogEntry) !== params.pluginId) return false;
  const officialInstall = (0, _officialExternalPluginCatalog64MlR6o.c)(catalogEntry);
...
function isSameGlobalPackageDuplicate(left, right) {
  ...
  const leftPackageName = (0, _stringCoerceBje8XVt.c)(left.packageName);
  const rightPackageName = (0, _stringCoerceBje8XVt.c)(right.packageName);
  if (!leftPackageName || leftPackageName !== rightPackageName) return false;
  const leftPackageVersion = (0, _stringCoerceBje8XVt.c)(left.packageVersion);
  const rightPackageVersion = (0, _stringCoerceBje8XVt.c)(right.packageVersion);
...
function loadPluginManifestRegistry(params = {}) {
  ...
  const currentHostVersion = (0, _versionDdTF4eka.o)(env);
  for (const candidate of candidates) {
    ...
    } : isBundleRecord && candidate.bundleFormat ? (0, _discoveryCVL9KJt.v)({
      rootDir: candidate.rootDir,
      bundleFormat: candidate.bundleFormat,
      rejectHardlinks
    }) : (0, _discoveryCVL9KJt.w)(candidate.rootDir, rejectHardlinks);
    if (!manifestRes.ok) {
      diagnostics.push({
        level: "error",
    ...
    if (candidate.origin !== "bundled") {
      ...
      const minHostVersionCheck = (0, _minHostVersionD6GrnMo.t)({
    ...
    if (existing) {
      ...
      if ((() => {
        ...
        const existingReal = (0, _discoveryCVL9KJt.f)(existing.candidate.rootDir, realpathCache);
        const candidateReal = (0, _discoveryCVL9KJt.f)(candidate.rootDir, realpathCache);
