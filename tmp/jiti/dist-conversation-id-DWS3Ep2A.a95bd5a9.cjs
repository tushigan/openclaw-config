"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = parseFeishuTargetId;exports.i = parseFeishuDirectConversationId;exports.n = buildFeishuModelOverrideParentCandidates;exports.r = parseFeishuConversationId;exports.t = buildFeishuConversationId;var _textRuntime = require("openclaw/plugin-sdk/text-runtime");
...
function buildFeishuModelOverrideParentCandidates(parentConversationId) {
  ...
  if (topicSenderMatch) {
    const chatId = (0, _textRuntime.normalizeLowercaseStringOrEmpty)(topicSenderMatch[1]);
    const topicId = (0, _textRuntime.normalizeLowercaseStringOrEmpty)(topicSenderMatch[2]);
    if (chatId && topicId) return [`${chatId}:topic:${topicId}`, chatId];
    return [];
  }
  const topicMatch = rawId.match(/^(.+):topic:([^:]+)$/i);
  if (topicMatch) {
    const chatId = (0, _textRuntime.normalizeLowercaseStringOrEmpty)(topicMatch[1]);
    return chatId ? [chatId] : [];
  }
  const senderMatch = rawId.match(/^(.+):sender:([^:]+)$/i);
  if (senderMatch) {
    const chatId = (0, _textRuntime.normalizeLowercaseStringOrEmpty)(senderMatch[1]);
    return chatId ? [chatId] : [];
  }
  return [];
}
//#endregion /* v9-dcb01af72f3fdf1c */
