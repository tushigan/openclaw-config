...
class ToolExecutionComponent extends _piTui.Container {
  ...
  constructor(toolName, toolCallId, args, options = {}, toolDefinition, ui, cwd) {
    ...
    this.builtInToolDefinition = (0, _index.createAllToolDefinitions)(cwd)[toolName];
    ...
    this.selfRenderContainer = new _piTui.Container();
  ...
  updateDisplay() {
    ...
    if (this.hasRendererDefinition()) {
      ...
      if (renderContainer instanceof _piTui.Box) {
    ...
    if (this.result) {
      ...
      for (let i = 0; i < imageBlocks.length; i++) {
        ...
        if (caps.images && this.showImages && img.data && img.mimeType) {
          ...
          const imageComponent = new _piTui.Image(imageData, imageMimeType, { fallbackColor: (s) => _theme.theme.fg("toolOutput", s) }, { maxWidthCells: this.imageWidthCells });
...
}exports.ToolExecutionComponent = ToolExecutionComponent; /* v9-2bbe3aa02a99ee7b */
