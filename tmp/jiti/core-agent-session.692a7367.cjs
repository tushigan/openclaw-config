...
class AgentSession {
  ...
  async _getRequiredRequestAuth(model) {
    ...
    if (!result.ok) {
      if (result.error.startsWith("No API key found")) {
        throw new Error((0, _authGuidance.formatNoApiKeyFoundMessage)(model.provider));
      }
  ...
  dispose() {
    ...
    (0, _piAi.cleanupSessionResources)(this.sessionId);
  }
  // =========================================================================
  ...
  _rebuildSystemPrompt(toolNames) {
    ...
    return (0, _systemPrompt.buildSystemPrompt)(this._baseSystemPromptOptions);
  }
  // =========================================================================
  // Prompting
  // =========================================================================
  ...
  async prompt(text, options) {
    ...
    try {
      ...
      // Validate model
      if (!this.model) {
        throw new Error((0, _authGuidance.formatNoModelSelectedMessage)());
      }
      if (!this._modelRegistry.hasConfiguredAuth(this.model)) {
        ...
        throw new Error((0, _authGuidance.formatNoApiKeyFoundMessage)(this.model.provider));
      }
  ...
  getAvailableThinkingLevels() {
    ...
    return (0, _piAi.getSupportedThinkingLevels)(this.model);
  }
  /**
  ...
  _clampThinkingLevel(level, _availableLevels) {
    return this.model ? (0, _piAi.clampThinkingLevel)(this.model, level) : "off";
  }
  // =========================================================================
  ...
  async compact(customInstructions) {
    ...
    try {
      ...
      } else
      {
        ...
        const result = await (0, _index.compact)(preparation, this.model, apiKey, headers, customInstructions, this._compactionAbortController.signal, this.thinkingLevel);
  ...
  async _runAutoCompaction(reason, willRetry) {
    ...
    try {
      ...
      {
        // Generate compaction result
        const compactResult = await (0, _index.compact)(preparation, this.model, apiKey, headers, undefined, this._autoCompactionAbortController.signal, this.thinkingLevel);
  ...
  _refreshToolRegistry(options) {
    ...
    filter((entry) => entry !== undefined));
    const runner = this._extensionRunner;
    const wrappedExtensionTools = (0, _index3.wrapRegisteredTools)(allCustomTools, runner);
    const wrappedBuiltInTools = (0, _index3.wrapRegisteredTools)(Array.from(this._baseToolDefinitions.values()).
    filter((definition) => isAllowedTool(definition.name)).
    map((definition) => ({
      definition,
      sourceInfo: (0, _sourceInfo.createSyntheticSourceInfo)(`<builtin:${definition.name}>`, { source: "builtin" })
    })), runner);
  ...
  _buildRuntime(options) {
    ...
    this._extensionRunner = new _index3.ExtensionRunner(extensionsResult.extensions, extensionsResult.runtime, this._cwd, this.sessionManager, this._modelRegistry);
  ...
  async reload() {
    ...
    await (0, _runner.emitSessionShutdownEvent)(this._extensionRunner, { type: "session_shutdown", reason: "reload" });
  ...
  async executeBash(command, onChunk, options) {
    ...
    try {
      const result = await (0, _bashExecutor.executeBashWithOperations)(resolvedCommand, this.sessionManager.getCwd(), options?.operations ?? (0, _bash.createLocalBashOperations)({ shellPath }), {
        onChunk,
...
}exports.AgentSession = AgentSession; /* v9-61444466afd5679f */
