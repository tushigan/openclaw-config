"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.i = resolveContextEngine;exports.n = registerContextEngine;exports.r = registerContextEngineForOwner;exports.t = clearContextEnginesForOwner;var _ansiDqm1lzVL = require("./ansi-Dqm1lzVL.js");
...
const contextEngineRegistryState = (0, _globalSingletonDZyLAEQq.n)(CONTEXT_ENGINE_REGISTRY_STATE, () => ({ engines: /* @__PURE__ */new Map() }));
...
function registerContextEngineForOwner(id, factory, owner, opts) {
  ...
  if (id === (0, _slotsCQkAb1S.n)("contextEngine") && normalizedOwner !== CORE_CONTEXT_ENGINE_OWNER) return {
    ok: false,
...
async function resolveContextEngine(config, options) {
  ...
  const engineId = typeof slotValue === "string" && slotValue.trim() ? slotValue.trim() : (0, _slotsCQkAb1S.n)("contextEngine");
  const defaultEngineId = (0, _slotsCQkAb1S.n)("contextEngine");
  ...
  if (!entry) {
    ...
    console.error(`[context-engine] Context engine "${(0, _ansiDqm1lzVL.t)(engineId)}" is not registered; falling back to default engine "${defaultEngineId}".`);
  ...
  try {
    ...
    console.error(`[context-engine] Context engine "${(0, _ansiDqm1lzVL.t)(engineId)}" factory threw during resolution: ${(0, _ansiDqm1lzVL.t)(factoryError instanceof Error ? factoryError.message : String(factoryError))}; falling back to default engine "${defaultEngineId}".`);
    return resolveDefaultContextEngine(defaultEngineId, factoryCtx);
  }
  let contractError;
  try {
    ...
    console.error(`[context-engine] Context engine "${(0, _ansiDqm1lzVL.t)(engineId)}" contract validation threw: ${(0, _ansiDqm1lzVL.t)(validationError instanceof Error ? validationError.message : String(validationError))}; falling back to default engine "${defaultEngineId}".`);
    return resolveDefaultContextEngine(defaultEngineId, factoryCtx);
  }
  if (contractError) {
    if (isDefaultEngine) throw new Error(contractError);
    console.error(`[context-engine] ${(0, _ansiDqm1lzVL.t)(contractError)}; falling back to default engine "${defaultEngineId}".`);
    return resolveDefaultContextEngine(defaultEngineId, factoryCtx);
  }
