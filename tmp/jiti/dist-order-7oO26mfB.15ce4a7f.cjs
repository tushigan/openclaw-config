"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = isActiveUnusableWindow;exports.c = resolveProfileUnusableUntil;exports.i = getSoonestCooldownExpiry;exports.n = resolveAuthProfileOrder;exports.o = isAuthCooldownBypassedForProvider;exports.r = clearExpiredCooldowns;exports.s = isProfileInCooldown;exports.t = resolveAuthProfileEligibility;var _providerIdDIRgKpoh = require("./provider-id-DIRgKpoh.js");
...
function resolveAuthProfileEligibility(params) {
  const providerAuthKey = (0, _providerAuthAliasesChhMyI_u.r)(params.provider, { config: params.cfg });
  const cred = params.store.profiles[params.profileId];
  if (!cred) return {
    eligible: false,
    reasonCode: "profile_missing"
  };
  if ((0, _providerAuthAliasesChhMyI_u.r)(cred.provider, { config: params.cfg }) !== providerAuthKey) return {
    eligible: false,
    reasonCode: "provider_mismatch"
  };
  const profileConfig = params.cfg?.auth?.profiles?.[params.profileId];
  if (profileConfig) {
    if ((0, _providerAuthAliasesChhMyI_u.r)(profileConfig.provider, { config: params.cfg }) !== providerAuthKey) return {
      eligible: false,
      reasonCode: "provider_mismatch"
    };
  ...
  const credentialEligibility = (0, _storeCSOk8Bno.T)({
    credential: cred,
    now: params.now
  });
  return {
...
function resolveAuthProfileOrder(params) {
  ...
  const providerKey = (0, _providerIdDIRgKpoh.r)(provider);
  const providerAuthKey = (0, _providerAuthAliasesChhMyI_u.r)(provider, { config: cfg });
  const now = Date.now();
  ...
  const explicitProfiles = cfg?.auth?.profiles ? Object.entries(cfg.auth.profiles).filter(([, profile]) => (0, _providerAuthAliasesChhMyI_u.r)(profile.provider, { config: cfg }) === providerAuthKey).map(([profileId]) => profileId) : [];
  const baseOrder = explicitOrder ?? (explicitProfiles.length > 0 ? explicitProfiles : (0, _profileListDRiyVI2U.n)(store, provider));
  ...
  if (filtered.length === 0 && explicitProfiles.length > 0 && allBaseProfilesMissing) filtered = (0, _profileListDRiyVI2U.n)(store, provider).filter(isValidProfile);
  const deduped = (0, _profileListDRiyVI2U.t)(filtered);
...
function resolveAuthOrder(order, provider) {
  return (0, _providerIdDIRgKpoh.n)(order, provider);
}
