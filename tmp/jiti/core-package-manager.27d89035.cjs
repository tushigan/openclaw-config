...
var _outputGuard = require("./output-guard.js");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}function getEnv() {if (process.platform !== "linux" || Object.keys(process.env).length > 0) {return process.env;}try {const data = (0, _nodeFs.readFileSync)("/proc/self/environ", "utf-8");const env = {};for (const entry of data.split("\0")) {const idx = entry.indexOf("=");if (idx > 0) {env[entry.slice(0, idx)] = entry.slice(idx + 1);}}return env;} catch {return process.env;}}
...
class DefaultPackageManager {
  ...
  async shouldUpdateNpmSource(source, scope) {
    ...
    const installedVersion = (0, _nodeFs.existsSync)(installedPath) ? this.getInstalledNpmVersion(installedPath) : undefined;
    if (!installedVersion) {
      return true;
    }
    try {
  ...
  getGlobalNpmRoot() {
    ...
    if (isBunPackageManager) {
      ...
      this.globalNpmRoot = (0, _nodePath.join)((0, _nodePath.dirname)(binDir), "install", "global", "node_modules");
    } else
    {
  ...
  collectFilesFromManifestEntries(entries, root, resourceType) {
    ...
    const resolved = sourceEntries.flatMap((entry) => {
      if (!hasGlobPattern(entry)) {
        return [(0, _nodePath.resolve)(root, entry)];
      }
      return (0, _glob.globSync)(entry, {
        cwd: root,
        absolute: true,
        dot: false,
        nodir: false
      }).map((match) => (0, _nodePath.resolve)(match));
    });
  ...
  toResolvedPaths(accumulator) {
    const mapToResolved = (entries) => {
      ...
      return resolved.filter((entry) => {
        const canonicalPath = (0, _paths.canonicalizePath)(entry.path);
  ...
  spawnCommand(command, args, options) {
    return (0, _nodeChild_process.spawn)(command, args, {
      cwd: options?.cwd,
...
}exports.DefaultPackageManager = DefaultPackageManager; /* v9-f12effccc22cefc5 */
