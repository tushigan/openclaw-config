...
const mediaAttachmentImageOpsLoader = (0, _lazyPromiseAiZRy56y.n)(async () => {
  const { loadBundledPluginPublicArtifactModuleSync } = await Promise.resolve().then(() => jitiImport("./public-surface-loader-PzS0nJLZ.js").then((m) => _interopRequireWildcard(m)));
...
async function withTempDir(fn) {
  const dir = await _promises.default.mkdtemp(_nodePath.default.join((0, _tmpOpenclawDirBT06rvao.n)(), "openclaw-img-"));
  try {
    return await fn(dir);
  } finally {
...
async function sipsMetadataFromBuffer(buffer) {
  return await withTempDir(async (dir) => {
    ...
    const { stdout } = await (0, _execKfr6njO_.i)("/usr/bin/sips", [
    "-g",
    "pixelWidth",
    "-g",
    "pixelHeight",
...
async function sipsResizeToJpeg(params) {
  return await withTempDir(async (dir) => {
    ...
    await (0, _execKfr6njO_.i)("/usr/bin/sips", [
    "-Z",
