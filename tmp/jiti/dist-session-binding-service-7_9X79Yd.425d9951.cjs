...
var _sessionBindingNormalizationBk9heWu = require("./session-binding-normalization-Bk9he-wu.js");
require("./json-store-DB5orcJO.js");
...
function buildConversationKey(ref) {
  const normalized = (0, _sessionBindingNormalizationBk9heWu.n)(ref);
  return [
  normalized.channel,
...
function resolveBindingsFilePath(env = process.env) {
  return _nodePath.default.join((0, _pathsC1_Y0cDn.v)(env), "bindings", "current-conversations.json");
}
...
function loadBindingsIntoMemory() {
  ...
  const parsed = (0, _jsonFileBDXsHiio.t)(resolveBindingsFilePath());
  ...
  for (const record of bindings ?? []) {
    ...
    const conversation = (0, _sessionBindingNormalizationBk9heWu.n)(record.conversation);
...
function persistBindingsToDisk() {
  (0, _jsonFileBDXsHiio.n)(resolveBindingsFilePath(), toPersistedFile());
}
...
function resolveChannelSupportsCurrentConversationBinding(channel) {
  const normalized = (0, _registryBy_qtZ6R.a)(channel) ?? (0, _stringCoerceBje8XVt.s)((0, _sessionBindingNormalizationBk9heWu.i)(channel));
  if (!normalized) return false;
  const matchesPluginId = (plugin) => plugin.id === normalized || (plugin.meta?.aliases ?? []).some((alias) => (0, _stringCoerceBje8XVt.s)(alias) === normalized);
  if (((0, _runtimeChannelStateC_dvLbjP.t)()?.channels ?? []).find((entry) => matchesPluginId(entry.plugin))?.plugin?.conversationBindings?.supportsCurrentConversationBinding === true) return true;
  return false;
}
...
async function bindGenericCurrentConversation(input) {
  const conversation = (0, _sessionBindingNormalizationBk9heWu.n)(input.conversation);
...
function toAdapterKey(params) {
  return (0, _sessionBindingNormalizationBk9heWu.t)(params);
}
...
const ADAPTERS_BY_CHANNEL_ACCOUNT = (0, _globalSingletonDZyLAEQq.t)(Symbol.for("openclaw.sessionBinding.adapters"));
...
function registerSessionBindingAdapter(adapter) {
  const normalizedAdapter = {
    ...adapter,
    ...(0, _sessionBindingNormalizationBk9heWu.n)({
...
function createDefaultSessionBindingService() {
  return {
    bind: async (input) => {
      const normalizedConversation = (0, _sessionBindingNormalizationBk9heWu.n)(input.conversation);
