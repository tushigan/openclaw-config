 ...
 */const ANSI_ESCAPE_REGEX = /\x1b\[[\d;]*m/g;function isBlankRenderedLine(line) {return line.replace(ANSI_ESCAPE_REGEX, "").trim().length === 0;}
  ...
  return {
    ...
    renderResult(toolCallId, toolName, result, details, isError) {
      try {
        ...
        const collapsed = (0, _ansiToHtml.ansiLinesToHtml)(trimRenderedResultLines(collapsedComponent.render(width)));
        // Render expanded
        ...
        const expanded = (0, _ansiToHtml.ansiLinesToHtml)(trimRenderedResultLines(expandedComponent.render(width)));
        return {
