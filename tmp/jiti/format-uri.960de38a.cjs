"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.IsUri = IsUri;function IsAlpha(ch) {
...
function IsUri(value) {
  ...
  if (value.charCodeAt(i) === 47 && value.charCodeAt(i + 1) === 47) {
    ...
    if (atPos !== -1) {
      ...
      for (let j = authorityStart; j < atPos; j++) {
        ...
        if (!IsUnreserved(ch) && !IsSubDelim(ch) && ch !== 58)
        return false;
      }
      i = atPos + 1;
    }
  ...
  while (i < length) {
    ...
    if (!(IsPchar(ch) || ch === 47 || ch === 63 || ch === 35)) {
      return false; // '/', '?', '#'
    }
    i++;
  }
