"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolveMirroredTranscriptText;exports.i = resolveSessionTranscriptFile;exports.n = appendExactAssistantMessageToSessionTranscript;exports.o = appendSessionTranscriptMessage;exports.r = readLatestAssistantTextFromSessionTranscript;exports.t = appendAssistantMessageToSessionTranscript;var _errorsQN8rySzW = require("./errors-QN8rySzW.js");
...
var _transcriptEventsAFLpOHJO = require("./transcript-events-aFLpOHJO.js");
var _chatMessageContentDY2n5MJD = require("./chat-message-content-DY2n5MJD.js");
var _sessionFileMHj2gKwZ = require("./session-file-MHj2gKwZ.js");
var _sessionWriteLockDCnT4uf = require("./session-write-lock-DCnT4uf1.js");
var _nodeFs = _interopRequireDefault(require("node:fs"));
var _nodePath = _interopRequireDefault(require("node:path"));
var _promises = _interopRequireDefault(require("node:fs/promises"));
var _nodeCrypto = require("node:crypto");
var _nodeString_decoder = require("node:string_decoder");
var _piCodingAgent = require("@mariozechner/pi-coding-agent");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}
...
async function appendSessionTranscriptMessage(params) {
  const lock = await (0, _sessionWriteLockDCnT4uf.r)({
    sessionFile: params.transcriptPath,
    timeoutMs: (0, _sessionWriteLockDCnT4uf.c)(params.config),
    allowReentrant: true
  });
  try {
...
async function loadPiCodingAgentModule() {
  piCodingAgentModulePromise ??= Promise.resolve().then(() => jitiImport("@mariozechner/pi-coding-agent").then((m) => _interopRequireWildcard(m)));
...
async function resolveSessionTranscriptFile(params) {
  const sessionPathOpts = (0, _pathsDG09LEN.o)({
    agentId: params.agentId,
    storePath: params.storePath
  });
  let sessionFile = (0, _pathsDG09LEN.a)(params.sessionId, params.sessionEntry, sessionPathOpts);
  let sessionEntry = params.sessionEntry;
  if (params.sessionStore && params.storePath) {
    const threadIdFromSessionKey = (0, _storeLoadDjtlQRVG.u)(params.sessionKey).threadId;
    const fallbackSessionFile = !sessionEntry?.sessionFile ? (0, _pathsDG09LEN.s)(params.sessionId, params.agentId, params.threadId ?? threadIdFromSessionKey) : void 0;
    const resolvedSessionFile = await (0, _sessionFileMHj2gKwZ.t)({
...
async function readLatestAssistantTextFromSessionTranscript(sessionFile) {
  ...
  let raw;
  try {
    raw = await _nodeFs.default.promises.readFile(sessionFile, "utf-8");
  } catch {
    return;
  }
  ...
  for (let index = lines.length - 1; index >= 0; index -= 1) {
    ...
    try {
      ...
      const text = (0, _chatMessageContentDY2n5MJD.r)(message)?.trim();
      if (!text) continue;
      return {
...
async function appendExactAssistantMessageToSessionTranscript(params) {
  ...
  const storePath = params.storePath ?? (0, _pathsDG09LEN.r)(params.agentId);
  const store = (0, _storeLoadDjtlQRVG.t)(storePath, { skipCache: true });
  const entry = store[(0, _storeDvxAjGkI.u)(sessionKey)] ?? store[sessionKey];
  ...
  let sessionFile;
  try {
    sessionFile = (await (0, _sessionFileMHj2gKwZ.t)({
    ...
    return {
      ok: false,
      reason: (0, _errorsQN8rySzW.i)(err)
    };
  }
