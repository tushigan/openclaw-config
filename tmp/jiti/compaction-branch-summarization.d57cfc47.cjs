...
async function generateBranchSummary(entries, options) {
  ...
  const response = await (0, _piAi.completeSimple)(model, { systemPrompt: _utils.SUMMARIZATION_SYSTEM_PROMPT, messages: summarizationMessages }, { apiKey, headers, signal, maxTokens: 2048 });
