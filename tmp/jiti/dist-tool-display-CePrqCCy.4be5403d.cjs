"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.i = void 0;exports.n = formatToolSummary;exports.r = resolveToolDisplay;exports.t = formatToolDetail;var _redact1fZUZMlV = require("./redact-1fZUZMlV.js");
...
function splitShellWords(input, maxWords = 48) {
  ...
  for (let i = 0; i < input.length; i += 1) {
    ...
    if (quote) {
      if (char === quote) quote = void 0;else
      current += char;
      continue;
    }
    if (char === "\"" || char === "'") {
...
function binaryName(token) {
  ...
  return (0, _stringCoerceBje8XVt.a)(cleaned.split(/[/]/).at(-1) ?? cleaned);
}
...
function stripShellPreamble(command) {
  ...
  for (let i = 0; i < 4; i += 1) {
    ...
    if (isChdir) if (isPopdCommand(head)) chdirPath = void 0;else
...
function resolveExecDetail(args, options) {
  const record = (0, _recordCoerceBjNW89OI.r)(args);
  if (!record) return;
...
function resolveActionArg(args) {
  ...
  return (0, _stringCoerceBje8XVt.c)(actionRaw) || void 0;
}
...
function coerceDisplayValue(value, opts = {}) {
  ...
  if (typeof value === "string") {
    const trimmed = value.trim();
    if (!trimmed) return;
    const firstLine = (0, _stringCoerceBje8XVt.c)(trimmed.split(/\r?\n/)[0]) ?? "";
    if (!firstLine) return;
...
function formatDetailKey(raw, overrides = {}) {
  ...
  return (0, _stringCoerceBje8XVt.a)(last.replace(/_/g, " ").replace(/-/g, " ").replace(/([a-z0-9])([A-Z])/g, "$1 $2")) || (0, _stringCoerceBje8XVt.a)(last);
}
...
function resolveWriteDetail(toolKey, args) {
  const record = (0, _recordCoerceBjNW89OI.r)(args);
  if (!record) return;
  const path = resolvePathArg(record) ?? (0, _stringCoerceBje8XVt.c)(record.url);
  if (!path) return;
...
function resolveWebSearchDetail(args) {
  const record = (0, _recordCoerceBjNW89OI.r)(args);
  if (!record) return;
  const query = (0, _stringCoerceBje8XVt.c)(record.query);
...
function resolveWebFetchDetail(args) {
  const record = (0, _recordCoerceBjNW89OI.r)(args);
  if (!record) return;
  const url = (0, _stringCoerceBje8XVt.c)(record.url);
  if (!url) return;
  const mode = (0, _stringCoerceBje8XVt.c)(record.extractMode);
...
const TOOL_DISPLAY_CONFIG = exports.i = {
  version: 1,
  fallback: {
    emoji: "🧩",
    detailKeys: [
...
function resolveToolDisplay(params) {
  ...
  const key = (0, _stringCoerceBje8XVt.a)(name);
  ...
  if (detail) detail = (0, _utilsD5swhEXt.h)(detail);
  return {
    name,
    emoji,
    title,
