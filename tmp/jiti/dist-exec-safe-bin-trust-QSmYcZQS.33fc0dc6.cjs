...
const DEFAULT_SAFE_BINS = exports.W = [
"cut",
"uniq",
"head",
"tail",
...
const SAFE_BIN_PROFILES = exports.G = compileSafeBinProfiles({
  jq: {
    maxPositional: 1,
    allowedValueFlags: [
    "--arg",
...
function normalizeSafeBinProfileName(raw) {
  const name = (0, _stringCoerceBje8XVt.a)(raw);
...
function normalizeMatchTarget(value) {
  if (process.platform === "win32") return (0, _stringCoerceBje8XVt.a)(value.replace(/^\\\\[?.]\\/, "").replace(/\\/g, "/"));
...
function matchesExecAllowlistPattern(pattern, target) {
  ...
  const expanded = trimmed.startsWith("~") ? (0, _homeDirG5LU3LmA.t)(trimmed) : trimmed;
...
function basenameLower(token) {
  ...
  return (0, _stringCoerceBje8XVt.a)(win.length < posix.length ? win : posix);
}
...
const COMMAND_CARRIER_EXECUTABLES = exports.R = new Set([
"sudo",
"doas",
"env",
"command",
...
function scanWrapperInvocation(argv, params) {
  ...
  while (idx < argv.length) {
    ...
    const directive = params.onToken(token, (0, _stringCoerceBje8XVt.a)(token));
...
function resolveInlineCommandMatch(argv, flags, options = {}) {
  for (let i = 1; i < argv.length; i += 1) {
    ...
    if (!token) continue;
    const lower = (0, _stringCoerceBje8XVt.a)(token);
    if (lower === "--") break;
    if (flags.has(lower)) {
...
function extractCmdInlineCommand(argv) {
  const idx = argv.findIndex((item) => {
    const token = (0, _stringCoerceBje8XVt.a)(item);
...
function resolveExecutablePathCandidate(rawExecutable, options) {
  const expanded = rawExecutable.startsWith("~") ? (0, _homeDirG5LU3LmA.t)(rawExecutable, { env: options?.env }) : rawExecutable;
...
function resolveWindowsExecutableExtensions(executable, env) {
  ...
  return ["", ...(env?.PATHEXT ?? env?.Pathext ?? process.env.PATHEXT ?? process.env.Pathext ?? ".EXE;.CMD;.BAT;.COM").split(";").map((ext) => (0, _stringCoerceBje8XVt.a)(ext))];
}
function resolveWindowsExecutableExtSet(env) {
  return new Set((env?.PATHEXT ?? env?.Pathext ?? process.env.PATHEXT ?? process.env.Pathext ?? ".EXE;.CMD;.BAT;.COM").split(";").map((ext) => (0, _stringCoerceBje8XVt.a)(ext)).filter(Boolean));
}
...
function matchArgPattern(argPattern, argv, platform) {
  ...
  try {
    ...
    if ((0, _stringCoerceBje8XVt.a)(platform ?? process.platform).startsWith("win")) {
...
function matchAllowlist(entries, resolution, argv, platform) {
  ...
  const useArgPattern = (0, _stringCoerceBje8XVt.a)(platform ?? process.platform).startsWith("win");
...
function normalizeSafeBinName(raw) {
  const trimmed = (0, _stringCoerceBje8XVt.a)(raw);
  if (!trimmed) return "";
