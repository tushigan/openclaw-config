"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports._ = formatStoppedCronSessionDiagnosticFields;exports.a = logRunAttempt;exports.b = markDiagnosticEmbeddedRunStarted;exports.c = logToolLoopAction;exports.d = logWebhookReceived;exports.f = markDiagnosticSessionProgress;exports.g = stopDiagnosticHeartbeat;exports.h = startDiagnosticHeartbeat;exports.i = logMessageQueued;exports.l = logWebhookError;exports.m = resolveStuckSessionWarnMs;exports.n = logActiveRuns;exports.o = logSessionAttention;exports.p = resetDiagnosticStateForTest;exports.r = logMessageProcessed;exports.s = logSessionStateChange;exports.t = getDiagnosticSessionStateCountForTest;exports.u = logWebhookProcessed;exports.v = resolveCronSessionDiagnosticContext;exports.y = markDiagnosticEmbeddedRunEnded;var _pathsC1_Y0cDn = require("./paths-C1_Y0cDn.js");
...
var _diagnosticPhaseCg1wuX5x = require("./diagnostic-phase-Cg1wuX5x.js");
var _diagnosticRuntimeJXOm4hMh = require("./diagnostic-runtime-jXOm4hMh.js");
var _diagnosticSessionState7K8_m5as = require("./diagnostic-session-state-7K8_m5as.js");
var _diagnosticStabilityBzDrfjbc = require("./diagnostic-stability-BzDrfjbc.js");
var _diagnosticStabilityBundleDWoaaVGd = require("./diagnostic-stability-bundle-DWoaaVGd.js");
var _nodeFs = _interopRequireDefault(require("node:fs"));
var _nodePath = _interopRequireDefault(require("node:path"));
var _nodePerf_hooks = require("node:perf_hooks");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}
...
function resolveSessionActivity(params) {
  ...
  for (const ref of sessionRefs(params)) {
    ...
    if (!byRef) continue;
    if (!activity) activity = byRef;else
    if (activity !== byRef) mergeSessionActivity(activity, byRef);
  }
  if (activity) {
...
function getDiagnosticSessionActivitySnapshot(params, now = Date.now()) {
  ...
  if (!activity) return {};
  let activeWorkKind;
  if (activity.activeTools.size > 0) activeWorkKind = "tool_call";else
  if (activity.activeModelCalls.size > 0) activeWorkKind = "model_call";else
  if (activity.activeEmbeddedRun) activeWorkKind = "embedded_run";
  let activeTool;
...
(0, _diagnosticEventsCjwOnQj.o)((event) => {
  switch (event.type) {
...
function emitDiagnosticMemorySample(options) {
  ...
  if (options?.emitSample !== false) (0, _diagnosticEventsCjwOnQj.n)({
  ...
  if (pressure && shouldEmitPressure(pressure, now, thresholds.pressureRepeatMs)) (0, _diagnosticEventsCjwOnQj.n)({
...
function resolveSessionFile(params) {
  ...
  return _nodePath.default.join((0, _pathsC1_Y0cDn.v)(), "agents", agentId, "sessions", `${runId}.jsonl`);
}
function readTailText(filePath) {
  ...
  try {
    ...
    const read = _nodeFs.default.readSync(fd, buffer, 0, length, start);
    return {
...
function readCronJobName(cronJobId) {
  if (!cronJobId) return;
  try {
    const raw = _nodeFs.default.readFileSync(_nodePath.default.join((0, _pathsC1_Y0cDn.v)(), "cron", "jobs.json"), "utf8");
...
function loadCommandPollBackoffRuntime() {
  commandPollBackoffRuntimePromise ??= Promise.resolve().then(() => jitiImport("./command-poll-backoff.runtime.js").then((m) => _interopRequireWildcard(m)));
  return commandPollBackoffRuntimePromise;
}
function recoverStuckSession(params) {
  stuckSessionRecoveryRuntimePromise ??= Promise.resolve().then(() => jitiImport("./diagnostic-stuck-session-recovery.runtime.js").then((m) => _interopRequireWildcard(m)));
  stuckSessionRecoveryRuntimePromise.then(({ recoverStuckDiagnosticSession }) => recoverStuckDiagnosticSession(params)).catch((err) => {
    _diagnosticRuntimeJXOm4hMh.t.warn(`stuck session recovery unavailable: ${String(err)}`);
  });
}
...
function getDiagnosticWorkSnapshot(now = Date.now()) {
  ...
  const waitingLabels = [];
  const queuedLabels = [];
  for (const state of _diagnosticSessionState7K8_m5as.t.values()) {
...
function hasRecentDiagnosticActivity(now) {
  const lastActivityAt = (0, _diagnosticRuntimeJXOm4hMh.n)();
...
function startDiagnosticLivenessSampler() {
  ...
  lastDiagnosticLivenessEventLoopUtilization = _nodePerf_hooks.performance.eventLoopUtilization();
  ...
  try {
    diagnosticLivenessMonitor = (0, _nodePerf_hooks.monitorEventLoopDelay)({ resolution: 20 });
    diagnosticLivenessMonitor.enable();
    diagnosticLivenessMonitor.reset();
  } catch (err) {
    diagnosticLivenessMonitor = null;
    _diagnosticRuntimeJXOm4hMh.t.debug(`diagnostic liveness monitor unavailable: ${String(err)}`);
  }
}
...
function sampleDiagnosticLiveness(now) {
  ...
  const currentEventLoopUtilization = _nodePerf_hooks.performance.eventLoopUtilization();
  const eventLoopUtilization = _nodePerf_hooks.performance.eventLoopUtilization(currentEventLoopUtilization, lastDiagnosticLivenessEventLoopUtilization).utilization;
...
function emitDiagnosticLivenessWarning(sample, work) {
  const phase = (0, _diagnosticPhaseCg1wuX5x.t)();
  const recentPhases = (0, _diagnosticPhaseCg1wuX5x.n)(6);
  ...
  if (hasBlockingWork || hasOpenDiagnosticWork(work) && hasSustainedEventLoopDelay) _diagnosticRuntimeJXOm4hMh.t.warn(message);else
  _diagnosticRuntimeJXOm4hMh.t.debug(message);
  ...
  (0, _diagnosticRuntimeJXOm4hMh.a)();
}
...
function logWebhookReceived(params) {
  ...
  if (_diagnosticRuntimeJXOm4hMh.t.isEnabled("debug")) _diagnosticRuntimeJXOm4hMh.t.debug(`webhook received: channel=${params.channel} type=${params.updateType ?? "unknown"} chatId=${params.chatId ?? "unknown"} total=${webhookStats.received}`);
  (0, _diagnosticEventsCjwOnQj.n)({
...
function logWebhookProcessed(params) {
  ...
  if (_diagnosticRuntimeJXOm4hMh.t.isEnabled("debug")) _diagnosticRuntimeJXOm4hMh.t.debug(`webhook processed: channel=${params.channel} type=${params.updateType ?? "unknown"} chatId=${params.chatId ?? "unknown"} duration=${params.durationMs ?? 0}ms processed=${webhookStats.processed}`);
  (0, _diagnosticEventsCjwOnQj.n)({
...
function logWebhookError(params) {
  ...
  _diagnosticRuntimeJXOm4hMh.t.error(`webhook error: channel=${params.channel} type=${params.updateType ?? "unknown"} chatId=${params.chatId ?? "unknown"} error="${params.error}" errors=${webhookStats.errors}`);
  (0, _diagnosticEventsCjwOnQj.n)({
    type: "webhook.error",
    channel: params.channel,
    updateType: params.updateType,
...
function logMessageQueued(params) {
  if (!(0, _diagnosticEventsCjwOnQj.t)()) return;
  const state = (0, _diagnosticSessionState7K8_m5as.n)(params);
  state.queueDepth += 1;
  state.lastActivity = Date.now();
  state.lastStuckWarnAgeMs = void 0;
  state.lastLongRunningWarnAgeMs = void 0;
  if (_diagnosticRuntimeJXOm4hMh.t.isEnabled("debug")) _diagnosticRuntimeJXOm4hMh.t.debug(`message queued: sessionId=${state.sessionId ?? "unknown"} sessionKey=${state.sessionKey ?? "unknown"} source=${params.source} queueDepth=${state.queueDepth} sessionState=${state.state}`);
  (0, _diagnosticEventsCjwOnQj.n)({
    type: "message.queued",
    sessionId: state.sessionId,
    sessionKey: state.sessionKey,
...
function logMessageProcessed(params) {
  if (!(0, _diagnosticEventsCjwOnQj.t)()) return;
  if (params.outcome === "error" ? _diagnosticRuntimeJXOm4hMh.t.isEnabled("error") : _diagnosticRuntimeJXOm4hMh.t.isEnabled("debug")) {
    ...
    if (params.outcome === "error") _diagnosticRuntimeJXOm4hMh.t.error(payload);else
    _diagnosticRuntimeJXOm4hMh.t.debug(payload);
  }
  (0, _diagnosticEventsCjwOnQj.n)({
...
function logSessionStateChange(params) {
  ...
  if (!isProbeSession && _diagnosticRuntimeJXOm4hMh.t.isEnabled("debug")) _diagnosticRuntimeJXOm4hMh.t.debug(`session state: sessionId=${state.sessionId ?? "unknown"} sessionKey=${state.sessionKey ?? "unknown"} prev=${prevState} new=${params.state} reason="${params.reason ?? ""}" queueDepth=${state.queueDepth}`);
  (0, _diagnosticEventsCjwOnQj.n)({
    type: "session.state",
    sessionId: state.sessionId,
    sessionKey: state.sessionKey,
...
function logSessionAttention(params) {
  ...
  if (classification.eventType === "session.long_running" && state.queueDepth <= 0) _diagnosticRuntimeJXOm4hMh.t.debug(message);else
  _diagnosticRuntimeJXOm4hMh.t.warn(message);
  const baseEvent = {
    sessionId: state.sessionId,
    sessionKey: state.sessionKey,
    state: params.state,
  ...
  if (classification.eventType === "session.long_running") (0, _diagnosticEventsCjwOnQj.n)({
    type: "session.long_running",
    ...baseEvent,
    classification: "long_running"
  });else
  if (classification.eventType === "session.stalled") (0, _diagnosticEventsCjwOnQj.n)({
...
function logRunAttempt(params) {
  if (!(0, _diagnosticEventsCjwOnQj.t)()) return;
  _diagnosticRuntimeJXOm4hMh.t.debug(`run attempt: sessionId=${params.sessionId ?? "unknown"} sessionKey=${params.sessionKey ?? "unknown"} runId=${params.runId} attempt=${params.attempt}`);
  (0, _diagnosticEventsCjwOnQj.n)({
    type: "run.attempt",
    sessionId: params.sessionId,
    sessionKey: params.sessionKey,
...
function logToolLoopAction(params) {
  ...
  if (params.level === "critical") _diagnosticRuntimeJXOm4hMh.t.error(payload);else
  _diagnosticRuntimeJXOm4hMh.t.warn(payload);
  (0, _diagnosticEventsCjwOnQj.n)({
    type: "tool.loop",
    sessionId: params.sessionId,
    sessionKey: params.sessionKey,
...
function logActiveRuns() {
  if (!(0, _diagnosticEventsCjwOnQj.t)()) return;
  const now = Date.now();
  const activeSessions = Array.from(_diagnosticSessionState7K8_m5as.t.entries()).filter(([, s]) => s.state === "processing").map(([id, s]) => `${id}(q=${s.queueDepth},age=${Math.round((now - s.lastActivity) / 1e3)}s)`);
  _diagnosticRuntimeJXOm4hMh.t.debug(`active runs: count=${activeSessions.length} sessions=[${activeSessions.join(", ")}]`);
  (0, _diagnosticRuntimeJXOm4hMh.a)();
}
let heartbeatInterval = null;
function startDiagnosticHeartbeat(config, opts) {
  if (!(0, _diagnosticEventsCjwOnQj.t)() || !(0, _diagnosticEventsCjwOnQj.i)(config)) return;
  (0, _diagnosticStabilityBzDrfjbc.o)();
  (0, _diagnosticStabilityBundleDWoaaVGd.a)();
  if (heartbeatInterval) return;
  startDiagnosticLivenessSampler();
  heartbeatInterval = setInterval(() => {
    let heartbeatConfig = config;
    if (!heartbeatConfig) try {
      heartbeatConfig = (opts?.getConfig ?? _ioE69J4lLI.i)();
    } catch {
      heartbeatConfig = void 0;
    }
    const stuckSessionWarnMs = resolveStuckSessionWarnMs(heartbeatConfig);
    const now = Date.now();
    (0, _diagnosticSessionState7K8_m5as.i)(now, true);
    ...
    _diagnosticRuntimeJXOm4hMh.t.debug(`heartbeat: webhooks=${webhookStats.received}/${webhookStats.processed}/${webhookStats.errors} active=${work.activeCount} waiting=${work.waitingCount} queued=${work.queuedCount}`);
    (0, _diagnosticEventsCjwOnQj.n)({
    ...
    loadCommandPollBackoffRuntime().then(({ pruneStaleCommandPolls }) => {
      for (const [, state] of _diagnosticSessionState7K8_m5as.t) pruneStaleCommandPolls(state);
    }).catch((err) => {
      _diagnosticRuntimeJXOm4hMh.t.debug(`command-poll-backoff prune failed: ${String(err)}`);
    });
    for (const [, state] of _diagnosticSessionState7K8_m5as.t) {
      ...
      if (state.state === "processing" && ageMs > stuckSessionWarnMs) {
        ...
        if (isStalledEmbeddedRunRecoveryEligible({
          classification,
          ageMs,
          stuckSessionWarnMs
...
function stopDiagnosticHeartbeat() {
  ...
  (0, _diagnosticStabilityBzDrfjbc.s)();
  (0, _diagnosticStabilityBundleDWoaaVGd.d)();
}
function getDiagnosticSessionStateCountForTest() {
  return (0, _diagnosticSessionState7K8_m5as.r)();
}
function resetDiagnosticStateForTest() {
  (0, _diagnosticSessionState7K8_m5as.a)();
  (0, _diagnosticRuntimeJXOm4hMh.o)();
  ...
  (0, _diagnosticPhaseCg1wuX5x.r)();
  (0, _diagnosticStabilityBzDrfjbc.i)();
  (0, _diagnosticStabilityBundleDWoaaVGd.l)();
}
//#endregion /* v9-5d3dc3da289566b8 */
