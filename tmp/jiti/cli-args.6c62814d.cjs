...
function parseArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if ((arg === "--tools" || arg === "-t") && i + 1 < args.length) {
      result.tools = args[++i].
      split(",").
      map((s) => s.trim()).
...
${_chalk.default.bold("Commands:")}
  ...
  ${_config.APP_NAME} update [source|self|pi]   Update pi and installed extensions
...
${_chalk.default.bold("Environment Variables:")}
  ...
  ${_config.ENV_AGENT_DIR.padEnd(32)} - Config directory (default: ~/${_config.CONFIG_DIR_NAME}/agent)
  ${_config.ENV_SESSION_DIR.padEnd(32)} - Session storage directory (overridden by --session-dir)
  PI_PACKAGE_DIR                   - Override package directory (for Nix/Guix store paths)
  PI_OFFLINE                       - Disable startup network operations when set to 1/true/yes
  PI_TELEMETRY                     - Override install telemetry when set to 1/true/yes or 0/false/no
  PI_SHARE_VIEWER_URL              - Base URL for /share command (default: https://pi.dev/session/)

${_chalk.default.bold("Built-in Tool Names:")}
