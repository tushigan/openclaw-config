"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.Diff = Diff;
var _index = require("../../guard/index.mjs");
var _index2 = require("../equal/index.mjs"); // deno-fmt-ignore-file
// ------------------------------------------------------------------
// Factory
// ------------------------------------------------------------------
...
function AssertCanDiffObject(value) {
  if (_index.Guard.IsObject(value) && _index.Guard.IsEqual(_index.Guard.Symbols(value).length, 0))
  return;
...
function* FromObject(path, left, right) {
  if (!_index.Guard.IsObject(right) || _index.Guard.IsArray(right))
  return yield CreateUpdate(path, right);
  AssertCanDiffObject(left);
  AssertCanDiffObject(right);
  const leftKeys = _index.Guard.Keys(left);
  const rightKeys = _index.Guard.Keys(right);
  // ----------------------------------------------------------------
  // Insert
  // ----------------------------------------------------------------
  for (const key of rightKeys) {
    if (_index.Guard.HasPropertyKey(left, key))
    continue;
  ...
  for (const key of leftKeys) {
    if (!_index.Guard.HasPropertyKey(right, key))
    continue;
    if (_index.Guard.IsUnsafePropertyKey(key))
    continue;
    if ((0, _index2.Equal)(left, right))
    continue;
...
function* FromTypedArray(path, left, right) {
  ...
  const predicate = _index.GlobalsGuard.IsTypeArray(right) &&
  _index.Guard.IsEqual(left.length, right.length) &&
  _index.Guard.IsEqual(typeLeft, typeRight);
  if (predicate) {
...
function* FromValue(path, left, right) {
  return _index.GlobalsGuard.IsTypeArray(left) ? yield* FromTypedArray(path, left, right) :
  _index.Guard.IsArray(left) ? yield* FromArray(path, left, right) :
  _index.Guard.IsObject(left) ? yield* FromObject(path, left, right) :
