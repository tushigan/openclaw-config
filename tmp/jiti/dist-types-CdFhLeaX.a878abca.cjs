...
var _dedupeBPv3m0hA = require("./dedupe-BPv3m0hA.js");
//#region src/plugins/command-registry-state.ts
const PLUGIN_COMMAND_STATE_KEY = Symbol.for("openclaw.pluginCommandsState");
const getState$1 = () => (0, _globalSingletonDZyLAEQq.n)(PLUGIN_COMMAND_STATE_KEY, () => ({
...
function restorePluginCommands(commands) {
  ...
  for (const command of commands) {
    const name = (0, _stringCoerceBje8XVt.s)(command.name);
    if (!name) continue;
...
function toPluginInteractiveRegistryKey(channel, namespace) {
  return `${(0, _stringCoerceBje8XVt.s)(channel) ?? ""}:${namespace.trim()}`;
}
...
function createInteractiveCallbackDedupe() {
  return (0, _dedupeBPv3m0hA.n)(PLUGIN_INTERACTIVE_CALLBACK_DEDUPE_KEY, {
    ttlMs: 5 * 6e4,
    maxSize: 4096
  });
}
...
function registerPluginInteractiveHandler(pluginId, registration, opts) {
  ...
  interactiveHandlers.set(key, {
    ...registration,
    namespace,
    channel: (0, _stringCoerceBje8XVt.s)(registration.channel) ?? "",
    pluginId,
...
function isReservedCommandName(name) {
  const trimmed = (0, _stringCoerceBje8XVt.s)(name) ?? "";
...
function validatePluginCommandDefinition(command, opts) {
  ...
  if (command.ownership === "reserved") {
    ...
    if (!isReservedCommandName(command.name)) return `Reserved command ownership requires a reserved command name: ${(0, _stringCoerceBje8XVt.s)(command.name) ?? ""}`;
  }
  ...
  if (command.requiredScopes !== void 0) {
    ...
    const unknownScope = command.requiredScopes.find((scope) => !(0, _operatorScopesD9nPeeKf.s)(scope));
...
function pluginCommandSupportsChannel(command, channel) {
  ...
  const normalizedChannel = (0, _stringCoerceBje8XVt.a)(channel);
  return command.channels.some((entry) => (0, _stringCoerceBje8XVt.a)(entry) === normalizedChannel);
}
function registerPluginCommand(pluginId, command, opts) {
  ...
  const normalizedName = (0, _stringCoerceBje8XVt.a)(name);
  const description = command.description.trim();
  const normalizedCommand = {
    ...command,
    name,
    description,
    ...(command.channels ? { channels: command.channels.map((channel) => (0, _stringCoerceBje8XVt.a)(channel)) } : {}),
  ...
  (0, _globalsCZuktVBk.r)(`Registered plugin command: ${key} (plugin: ${pluginId})`);
  return { ok: true };
}
//#endregion
