...
function isThreadSessionKey(sessionKey) {
  return Boolean((0, _storeLoadDjtlQRVG.f)(sessionKey).threadId);
}
function resolveSessionResetType(params) {
  ...
  const normalized = (0, _stringCoerceBje8XVt.a)(params.sessionKey);
...
function resolveChannelResetConfig(params) {
  ...
  const normalized = (0, _messageChannelB1O_CztY.u)(params.channel);
  const fallback = (0, _stringCoerceBje8XVt.s)(params.channel);
  const key = normalized ?? fallback;
  if (!key) return;
  return resetByChannel[key];
}
//#endregion /* v9-61114f52852278f7 */
