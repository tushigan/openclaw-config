...
function getOfficialExternalPluginCatalogManifest(entry) {
  ...
  return (0, _utilsD5swhEXt.c)(manifest) ? manifest : void 0;
}
function resolveOfficialExternalPluginId(entry) {
  const manifest = getOfficialExternalPluginCatalogManifest(entry);
  return (0, _stringCoerceBje8XVt.c)(manifest?.plugin?.id) ?? (0, _stringCoerceBje8XVt.c)(manifest?.channel?.id) ?? (0, _stringCoerceBje8XVt.c)(manifest?.providers?.[0]?.id);
}
function resolveOfficialExternalPluginLookupIds(entry) {
  const manifest = getOfficialExternalPluginCatalogManifest(entry);
  return [
  (0, _stringCoerceBje8XVt.c)(manifest?.plugin?.id),
  (0, _stringCoerceBje8XVt.c)(manifest?.channel?.id),
  (0, _stringCoerceBje8XVt.c)(manifest?.providers?.[0]?.id)].
  filter((value, index, all) => Boolean(value) && all.indexOf(value) === index);
}
function resolveOfficialExternalPluginLabel(entry) {
  const manifest = getOfficialExternalPluginCatalogManifest(entry);
  return (0, _stringCoerceBje8XVt.c)(manifest?.plugin?.label) ?? (0, _stringCoerceBje8XVt.c)(manifest?.channel?.label) ?? (0, _stringCoerceBje8XVt.c)(manifest?.providers?.[0]?.name) ?? (0, _stringCoerceBje8XVt.c)(entry.name) ?? resolveOfficialExternalPluginId(entry) ?? "plugin";
}
function resolveOfficialExternalPluginInstall(entry) {
  ...
  const npmSpec = (0, _stringCoerceBje8XVt.c)(install?.npmSpec) ?? (0, _stringCoerceBje8XVt.c)(entry.name);
...
function getOfficialExternalPluginCatalogEntryForPackage(packageName) {
  ...
  if (!normalized) return;
  return listOfficialExternalPluginCatalogEntries().find((entry) => (0, _stringCoerceBje8XVt.c)(entry.name) === normalized);
}
//#endregion /* v9-90b62e4091e51886 */
