"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.t = formatToolAggregate;var _stringCoerceBje8XVt = require("./string-coerce-Bje8XVt9.js");
...
function shortenMeta(meta) {
  if (!meta) return meta;
  return (0, _utilsD5swhEXt.h)(meta);
}
function formatToolAggregate(toolName, metas, options) {
  const filtered = (metas ?? []).filter(Boolean).map(shortenMeta);
  const display = (0, _toolDisplayCePrqCCy.r)({ name: toolName });
...
function formatMetaForDisplay(toolName, meta, markdown) {
  const normalized = (0, _stringCoerceBje8XVt.a)(toolName);
