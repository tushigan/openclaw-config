...
function resolveOfficialCatalogPaths(options) {
  ...
  const candidates = [(0, _openclawRootCRSCIPqz.n)({ cwd: process.cwd() }), (0, _openclawRootCRSCIPqz.n)({ moduleUrl: "file:///opt/homebrew/lib/node_modules/openclaw/dist/catalog-DYcB5w-_.js" })].filter((entry, index, all) => Boolean(entry) && all.indexOf(entry) === index).map((packageRoot) => _nodePath.default.join(packageRoot, OFFICIAL_CHANNEL_CATALOG_RELATIVE_PATH));
  if (process.execPath) {
