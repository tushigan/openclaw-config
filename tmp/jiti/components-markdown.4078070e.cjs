...
class StrictStrikethroughTokenizer extends _marked.Tokenizer {
  del(src) {
...
const markdownParser = new _marked.Marked();
    ...
    for (const token of tokens) {
      switch (token.type) {
        ...
        case "link":{
            ...
            if ((0, _terminalImage.getCapabilities)().hyperlinks) {
              // OSC 8: render as a clickable hyperlink. The URL is not printed inline,
              // so we always show only the link text regardless of whether it matches href.
              result += (0, _terminalImage.hyperlink)(styledLink, token.href) + stylePrefix;
            } else
            {
...
}exports.Markdown = Markdown; /* v9-88232fa46658b91c */
