...
function isWorkspacePluginTrustedForProviderEnvVars(plugin, config) {
  return (0, _providerAuthAliasesChhMyI_u.i)({
    config,
    isImplicitlyAllowed: (pluginId) => (0, _slotsCQkAb1S.r)(plugin.kind, "context-engine") && (0, _providerAuthAliasesChhMyI_u.a)(config?.plugins?.slots?.contextEngine) === pluginId,
    plugin
  });
}
...
function loadProviderEnvMetadataSnapshot(params) {
  ...
  return (0, _currentPluginMetadataSnapshotCBeGKry.n)({
    config,
    env,
...
function resolveManifestProviderAuthEnvVarCandidates(params) {
  ...
  const aliases = (0, _providerAuthAliasesChhMyI_u.n)(params);
...
function resolveManifestProviderAuthEvidence(params) {
  ...
  for (const plugin of snapshot.plugins) {
    if (snapshot.index.plugins.length > 0 && !(0, _installedPluginIndexStoreDH9sPamj.d)(snapshot.index, plugin.id, params?.config)) continue;
