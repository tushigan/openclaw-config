...
var _zodParseByT__FkO = require("./zod-parse-ByT__FkO.js");
...
function resolveInstalledPluginIndexPolicyHash(config) {
  const normalized = (0, _configStateWKtsQXM.s)(config?.plugins);
  const channelPolicy = {};
...
function buildStartupInfo(record) {
  return {
    ...
    memory: (0, _slotsCQkAb1S.r)(record.kind, "memory"),
...
function resolvePackageJsonPath(candidate) {
  ...
  const packageDir = (0, _discoveryCVL9KJt.f)(candidate.packageDir) ?? _nodePath.default.resolve(candidate.packageDir);
...
function resolvePackageJsonRelativePath(rootDir, packageJsonPath) {
  const resolvedRootDir = (0, _discoveryCVL9KJt.f)(rootDir) ?? _nodePath.default.resolve(rootDir);
...
function describePackageInstallSource(candidate) {
  ...
  if (!install) return;
  return (0, _installSourceInfoD4ewXUGX.t)(install, { expectedPackageName: candidate?.packageName });
}
...
function buildInstalledPluginIndexRecords(params) {
  ...
  const normalizedConfig = (0, _configStateWKtsQXM.s)(params.config?.plugins);
  return params.registry.plugins.map((record) => {
    ...
    const enabled = (0, _configStateWKtsQXM.c)({
      id: record.id,
...
function resolveInstalledPluginIndexRegistry(params) {
  if (params.candidates) return {
    ...
    registry: (0, _manifestRegistryBiAsJcRZ.t)({
  ...
  const normalized = (0, _configStateWKtsQXM.s)(params.config?.plugins);
  const installRecords = params.installRecords ?? (0, _manifestRegistryBiAsJcRZ.s)({ env: params.env });
...
function buildInstalledPluginIndex(params) {
  ...
  const installRecords = normalizeInstallRecordMap(params.installRecords ?? (0, _manifestRegistryBiAsJcRZ.s)({
    env,
  ...
  return {
    ...
    hostContractVersion: (0, _versionDdTF4eka.o)(env),
...
function isInstalledPluginEnabled(index, pluginId, config) {
  ...
  const normalizedConfig = (0, _configStateWKtsQXM.s)(config?.plugins);
  const state = (0, _configStateWKtsQXM.l)({
    id: record.pluginId,
    origin: record.origin,
    config: normalizedConfig,
    rootConfig: config,
...
function copySafeInstallRecords(records) {
  ...
  for (const [pluginId, record] of Object.entries(records)) {
    if ((0, _prototypeKeysBWjW0VW.t)(pluginId)) continue;
    safeRecords[pluginId] = record;
  }
  return safeRecords;
}
function parseInstalledPluginIndex(value) {
  const parsed = (0, _zodParseByT__FkO.n)(InstalledPluginIndexSchema, value);
...
async function readPersistedInstalledPluginIndex(options = {}) {
  return parseInstalledPluginIndex(await (0, _jsonFilesDPM4MwsB.i)((0, _manifestRegistryBiAsJcRZ.u)(options)));
}
function readPersistedInstalledPluginIndexSync(options = {}) {
  return parseInstalledPluginIndex((0, _jsonFilesDPM4MwsB.a)((0, _manifestRegistryBiAsJcRZ.u)(options)));
}
async function writePersistedInstalledPluginIndex(index, options = {}) {
  const filePath = (0, _manifestRegistryBiAsJcRZ.u)(options);
  await (0, _jsonFilesDPM4MwsB.o)(filePath, {
    ...index,
    warning: INSTALLED_PLUGIN_INDEX_WARNING
  }, {
...
function writePersistedInstalledPluginIndexSync(index, options = {}) {
  const filePath = (0, _manifestRegistryBiAsJcRZ.u)(options);
  (0, _jsonFileBDXsHiio.n)(filePath, {
    ...index,
    warning: INSTALLED_PLUGIN_INDEX_WARNING
  });
  clearCurrentPluginMetadataSnapshotState();
...
function canRefreshPersistedPolicyState(persisted, params) {
  ...
  const env = params.env ?? process.env;
  if (persisted.version !== 1 || persisted.hostContractVersion !== (0, _versionDdTF4eka.o)(env) || persisted.compatRegistryVersion !== resolveCompatRegistryVersion() || persisted.migrationVersion !== 1) return false;
...
function refreshPersistedPolicyState(persisted, params) {
  ...
  return {
    ...
    plugins: persisted.plugins.map((plugin) => ({
      ...plugin,
      enabled: (0, _configStateWKtsQXM.c)({
        id: plugin.pluginId,
