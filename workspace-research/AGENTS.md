...
- 涉及真实网页操作、平台站内搜索、登录态、翻页、点击、动态页面、抖音/小红书/淘宝/天猫/京东等平台抓取时，必须先读并使用 `browser-automation`。如果任务明确需要登录态，先检查 `browser` profiles；`user` profile 不可用时，直接说明“用户登录态浏览器未接通”，不得退化为匿名 headless 抓取后声称完成。
...
- 平台站内搜索/爬取必须区分三种状态：真实登录态已接通、匿名浏览器可访问、被风控/验证码/空壳拦截。只有第一种能回答“用我的登录态抓取”；第二、三种只能作为失败证据或候选入口，不能当作完成结果。
