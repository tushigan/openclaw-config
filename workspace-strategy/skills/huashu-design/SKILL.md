...
1. **理解需求**：
   ...
   - 🛑 **幻灯片/PPT 任务：HTML 聚合演示版永远是默认基础产物**（不管用户最终要什么格式）：
     ...
     - 🔴 **大型 PPT（≥10 页）生成策略（强制执行）**：当 PPT 页数 ≥10 页时，**禁止尝试一次性生成完整 HTML**。必须采用**分批逐页生成**策略：
       - **第一步：创建目录结构**：`mkdir -p 项目名/slides`
       - **第二步：分批生成（每批 3-5 页）**：逐批调用 `write` 工具生成独立页面文件（`slides/slide-01.html`, `slides/slide-02.html`, ...），每个文件包含完整的 1920×1080 单页 HTML
       - **第三步：每批完成后告知用户进度**：「已完成第 1-5 页」「已完成第 6-10 页」，保持用户知情
       - **第四步：全部页面完成后生成 index.html**：复制 `assets/deck_index.html` 为 `index.html`，编辑 MANIFEST 数组列出所有页面
       - **为什么必须分批**：单次 `write` 工具调用的 `content` 参数有实际大小限制。18 页完整 HTML 预计 5000-8000 行代码，超出单次工具调用能力范围，会导致工具调用时 `content` 参数丢失而连续失败。分批生成每次只需生成 1 页（约 200-400 行），完全在工具能力范围内。
       - **错误恢复**：如果某一页 `write` 调用失败，只需重试该单页，不影响已完成的其他页面。
...
| **大型 PPT write 工具连续失败** | **生成 ≥10 页 PPT 时，write 工具连续 2 次以上调用失败（提示缺少 content 参数或内容过大）** | **立即停止重试，改用分批生成策略**：① 告知用户「检测到 write 工具限制，切换到分批生成模式」；② 按「工作流程 Step 1」中的大型 PPT 生成策略执行（每批 3-5 页，逐批调用 write）；③ 每批完成后告知进度；④ 全部完成后生成 index.html。**禁止**：连续 3 次以上用同样方式重试、尝试用 heredoc 绕过（违反 skill 规范）、放弃任务。**根因**：单次 write 调用的 content 参数有实际大小限制（约 3000-5000 行），18 页 PPT 预计 5000-8000 行会超限 |
...
| `scripts/gen_large_deck.py` | **大型 PPT 快速生成辅助工具**（≥10 页 PPT 的备选方案）| 从 Markdown 脚本逐页生成独立 HTML + 自动生成 index.html。**纯 Python 标准库实现，无外部依赖**。用法：`python3 gen_large_deck.py --script ppt_script.md --output deck/ --style modern-minimal`。支持 3 种内置样式：modern-minimal（默认）/ dark-elegant / warm-creative。⚠️ 这是**备选工具**，优先使用 write 工具分批生成以保持设计质量；仅当需要快速原型或用户提供了规范 Markdown 脚本时使用 |
