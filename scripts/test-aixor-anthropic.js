...
/**
 * Aixor 端点性能测试脚本（Anthropic Messages API）
 */
...
// 测试配置
const ENDPOINTS = [
  { name: 'aixor.org', url: 'https://aixor.org/v1/messages' },
  { name: 'cn.aixor.org', url: 'https://cn.aixor.org/v1/messages' }
];

const TEST_ROUNDS = 10;
const TIMEOUT = 10000;

const API_KEY = process.env.AIXOR_API_KEY || process.argv[2];

if (!API_KEY) {
  console.error('❌ 错误：需要提供 API Key');
  console.error('使用方式：');
  console.error('  AIXOR_API_KEY=your_key node test-aixor-anthropic.js');
  console.error('  或者：node test-aixor-anthropic.js your_key');
  process.exit(1);
}
...
async function runTests() {
  console.log('🧪 开始 Aixor 端点性能测试（Anthropic API）\n');
  console.log(`📊 测试配置：`);
  ...
  const speedDiff = worst.avgDuration - best.avgDuration;
  if (speedDiff > 0) {
    console.log(`  ✓ 比 ${worst.name} 快 ${speedDiff}ms（${((speedDiff / worst.avgDuration) * 100).toFixed(1)}%）`);
  }
