...
Step 6：调用 prompt-assembler 组装完整 prompt → 询问本次抽卡张数
Step 7：派发 design subagent → 按确认张数生图（默认 1 张，最多 3 张）
Step 8：发送成品图 → 用户确认
...
| 多方向出图 | 不要手写 `prompt_A/B/C.txt` 直接生图 | 先回到创意确认；单项目只保留一个确认方向，多方向必须拆成多个项目 |
...
**未得到用户明确确认前，禁止进入 Step 5.5（风格提炼）或 Step 5.8（创意表达）。**

确认方式：
- 用户回复「确认文案」或「OK」或「可以」或「没问题」
- 或用户提出具体修改要求后再次确认
...
- 如果用户说“三个都要 / 每个方案各出”，先回到创意确认，只保留一个确认方向后再抽卡；不得手写 `prompt_A/B/C.txt` 直接生图
- 最多 3 张；如果用户回复超过 3 张，一律按 3 张执行
```
...
- 已确认本次抽卡张数；若用户只回复「确认生图 / 确认 / OK / 可以」，视为默认 `1` 张；若用户回复大于 `3`，必须截断为 `3`
...
python3 "${EXEC_SCRIPT}" --project-dir "${PROJECT_DIR}" --size 2160x3840 --aspect 9:16 --model gpt-image-2 --count "${DRAW_COUNT}"
```

**注意**：
- 如果用户要求“一次多抽几张 / 给 N 个备选 / 多来几版”，main 必须把数量写进 `run.sh` 的 `--count N`。底层 `gpt-image2-gen` 会并行生成，不要手写串行循环。
...
- `DRAW_COUNT` 来自 Step 6 的用户确认：默认 `1`，最多 `3`；用户要求超过 `3` 时写入 `3`
...
3. 如果 `generation_result.json` 中 `ok=true`，再确认 `generated_paths` 中的所有输出文件都存在。默认 1 张时至少存在：
   /Users/a123/.openclaw/workspace/brand-poster-projects/[任务ID]/images/final_poster.png

4. 完成后只回传：`generation_result.json` 路径；若成功，再附上 `generated_paths` 中所有海报路径。
...
2. 再检查 `generated_paths` 中所有图片是否真实存在；如果旧结果没有 `generated_paths`，至少检查 `images/final_poster.png`
3. 确认每个文件大小合理（不应为 0 字节）
...
仅当 `generation_result.json.ok=true` 且成品文件真实存在后，才将成品图发送给用户确认。如果本次抽卡多张，必须把所有候选图都纳入交付清单并真实发送，不得只发送第一张。
...
9. 若 `deliverables.candidate_images` 存在多张候选，必须按其中每个 `send_path` 逐张调用真实媒体发送，并记录所有 `sent_paths`
10. 任何”局部修改””继续调整””重新生成”都必须继续引用 `edit_source_image` 指向的原图，不得把 preview 图当作修改输入
