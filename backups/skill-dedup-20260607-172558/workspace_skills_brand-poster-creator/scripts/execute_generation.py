...
def clamp_draw_count(value: int | str | None) -> int:
    try:
...
def expected_output_paths(output_path: Path, count: int) -> list[Path]:
    return [
        output_path if idx == 1 else output_path.with_name(f'{output_path.stem}_{idx}{output_path.suffix}')
        for idx in range(1, count + 1)
    ]


def build_command(prompt_path: Path, output_path: Path, refs: list[dict], size: str, aspect: str, model: str, count: int) -> list[str]:
    cmd = [
        'python3',
        str(TARGET_SCRIPT),
        '--prompt-file',
...
def make_base_result(project_dir: Path, attempt: int, prompt_path: Path, output_path: Path, stdout_log: Path, stderr_log: Path, refs: list[dict], missing_refs: list[str], command_summary: str, count: int = 1) -> dict:
    output_paths = expected_output_paths(output_path, clamp_draw_count(count))
    return {
        'attempt': attempt,
        ...
        'draw_count': clamp_draw_count(count),
...
def main() -> int:
    ...
    parser.add_argument('--count', type=int, default=1, help='本次抽卡张数，默认 1，最多 3；超过 3 自动按 3 执行')
    args = parser.parse_args()
    draw_count = clamp_draw_count(args.count)
    ...
    if not TARGET_SCRIPT.exists():
        base = make_base_result(project_dir, attempt, prompt_path, output_path, stdout_log, stderr_log, [], [], '', draw_count)
        base.update({
            'status': 'failed',
    ...
    cmd = build_command(prompt_path, output_path, resolved_refs, args.size, args.aspect, args.model, draw_count) if not errors else []
    command_summary = summarize_command(cmd) if cmd else ''
    generation_env = build_generation_env(brief)

    started_payload = make_base_result(project_dir, attempt, prompt_path, output_path, stdout_log, stderr_log, resolved_refs, missing_refs, command_summary, draw_count)
    ...
    output_paths = expected_output_paths(output_path, draw_count)
    existing_outputs = [p for p in output_paths if p.exists() and p.stat().st_size > 0]
    exists = len(existing_outputs) == draw_count
    size = sum(p.stat().st_size for p in existing_outputs)
    ...
    payload.update({
        ...
        'ok': True,
        'generated_paths': [str(p) for p in output_paths],
    ...
    manager.complete_stage(
        ...
        files=[manager._relative(result_path), manager._relative(stdout_log), manager._relative(stderr_log)] + [manager._relative(p) for p in output_paths],
        extra={'output_size': size, 'exit_code': proc.returncode, 'draw_count': draw_count},
    )
    return 0
