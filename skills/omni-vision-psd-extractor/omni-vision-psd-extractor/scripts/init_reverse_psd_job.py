...
def main() -> None:
    ...
    manifest_layers = []
    for layer in layers:
        width_val = int(layer["width"])
        height_val = int(layer["height"])
        if max(width_val, height_val) < 200 and layer["group"] not in ["02_LOGO", "03_TITLE"]:
            print(f"[info] Layer '{layer['key']}' (size={width_val}x{height_val}) is less than 200px. Filtering and merging into background to prevent resource waste.")
            continue
