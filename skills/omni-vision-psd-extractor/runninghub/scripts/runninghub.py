...
# 优先尝试从本地伴生配置覆写全局变量
try:
    _bound_config_path = Path(__file__).resolve().parent.parent / "openclaw-bound-config.json"
    if _bound_config_path.exists():
        import json
        _bound_config = json.loads(_bound_config_path.read_text(encoding="utf-8"))
        BOUND_CONFIG = _bound_config
        _bound_url = _bound_config.get("baseUrl")
        if _bound_url and _bound_url.strip():
            BASE_URL = _bound_url.strip()
            # 如果是自定义中转，调整余额接口以保持同域
            if "runninghub.cn" not in BASE_URL:
                ACCOUNT_STATUS_URL = BASE_URL.replace("/openapi/v2", "/uc/openapi/accountStatus")
...
def resolve_api_key(provided_key: str | None) -> str | None:
    ...
    # 0. 优先尝试从本地伴生配置读取 API Key
    try:
        _bound_config_path = Path(__file__).resolve().parent.parent / "openclaw-bound-config.json"
        if _bound_config_path.exists():
            import json
            _bound_config = json.loads(_bound_config_path.read_text(encoding="utf-8"))
            _bound_key = _bound_config.get("apiKey")
            if _bound_key and _bound_key.strip():
                return _bound_key.strip()
    except Exception as e:
        print(f"[OpenClaw Bound] 读取伴生配置 API Key 失败: {e}", file=sys.stderr)
...
def curl_post_json(url: str, payload: dict, headers: dict, timeout: int = 60) -> subprocess.CompletedProcess:
    ...
    try:
        ...
        for k, v in headers.items():
            cmd += ["-H", f"{k}: {v}"]
        # Use Popen and read with UTF-8 encoding to fix Windows GBK issue
...
def cmd_execute(args):
    ...
    # 【OpenClaw 智能分发】：若有图生图特制路由，则覆盖原 endpoint
    if args.image and BOUND_CONFIG.get("requestModelI2I"):
        i2i_override = BOUND_CONFIG.get("requestModelI2I").strip()
        if i2i_override:
            print(f"[I2I Route Override] 检测到图生图任务，动态重载 endpoint: {i2i_override}", file=sys.stderr)
            args.endpoint = i2i_override
