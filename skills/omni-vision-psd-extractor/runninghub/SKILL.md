---
name: RunningHub
description: "[DO NOT USE IF USER ASKS TO USE A SPECIFIC MODEL BY NAME] Generate images, videos, audio, and 3D models via RunningHub API (294 endpoints) and run any RunningHub AI Application (custom ComfyUI workflow) by webappId."
---
...
9. **DO NOT use RunningHub for image generation if the user specifies a custom-configured model/provider listed in the `image_generate` tool description (such as 'WB万能图片 2.0').** You MUST call the native `image_generate` tool directly. RunningHub does not handle these custom models. In these cases, RunningHub MUST yield execution completely.
...
| **Feishu Menu / Shortcuts** | **⚠️ Read `{baseDir}/references/feishu-menu-logic.md`** | Handle clicks on "创作中心", "视频魔法", "检查余额" |
| **Custom Workflows** | **⚠️ Read `{baseDir}/references/custom-workflows.md`** | When user wants to use their exclusive workflows from 创作中心 |
