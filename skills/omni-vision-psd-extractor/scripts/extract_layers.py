...
def run_bound_gemini(prompt, source_path, raw_out, env, suggested_size, source_url=None):
    """
    调用图像生成 API，支持 OpenAI 和 Gemini 两种协议格式。
    ...
    参数:
        ...
        source_url: 可选，已有的 Cloudinary URL
    ...
    if not config["api_key"]:
        error = "OPENCLAW_BOUND_API_KEY is missing for image generation."
    ...
    # 根据协议类型选择不同的处理方式
    if config.get("protocol") == "openai":
        # OpenAI /v1/images/edits 格式（gpt-image-2-pro）
        return _run_openai_image_edits(prompt, source_path, raw_out, config, suggested_size, result_path)
    else:
        # Gemini API 格式
        return _run_gemini_generate(prompt, source_path, raw_out, config, suggested_size, source_url, result_path)


def _run_openai_image_edits(prompt, source_path, raw_out, config, suggested_size, result_path):
    """
    ...
    body_parts.append(b'')
    body_parts.append(config["model"].encode('utf-8'))

    # size 字段
    body_parts.append(f'--{boundary}'.encode())
    body_parts.append(b'Content-Disposition: form-data; name="size"')
    body_parts.append(b'')
    body_parts.append(size_param.encode('utf-8'))
    ...
    # 解析 OpenAI 格式的响应: {"data": [{"url": "..."}, ...]}
    if "data" not in data or not data["data"]:
        last_error = "OpenAI API response did not contain image data."
        result_path.write_text(json.dumps({
            "status": "error",
            "api_provider": config["provider"],
            "api_base_url": config["base_url"],
...
def process_single_task(task, args, env, generator_path, remove_chroma_script, manifest, source_path, state, state_path, anchor_key, source_url=None, is_parallel=True):
    ...
    if feishu_user or feishu_chat:
        if task["kind"] == "background":
            api_result_label = f"【API 第 1 次请求完成】背景层提取结果"
        else:
            api_result_label = f"【API 第 2 次请求完成】前景层元素整理结果（绿幕背景）"

        safe_log(f"[{key}] 📤 立即发送 API 原始结果到飞书: {api_result_label}")
        send_feishu_image(raw_out, feishu_user, feishu_chat, key, api_result_label, f"API-{completed_count+1}/{total_parallel_tasks}" if is_parallel else "API-1/2")
    ...
    # 注意：图片已在 API 完成后和去绿幕完成后分别发送到飞书，此处不再重复发送
