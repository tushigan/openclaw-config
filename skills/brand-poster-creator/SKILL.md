...
1. 读取对应蒸馏卡 JSON 文件：`/Users/a123/.openclaw/workspace/skills/brand-poster-distiller/cards/[ID].json`
2. 将完整 JSON 数据写入 `distill_card.json`
3. 提取 `layout_analysis.elements` 作为版式坐标数据
4. 提取 `layout_analysis.copy_planning_guide` 作为文案策划指南
5. 提取 `layout_analysis.negative_constraints` 作为负面约束
6. 将骨架图复制到 `images/skeleton.png`：优先使用 `skeleton_png` 字段（PNG），若为空则回退 `skeleton_image` 字段（SVG）
...
2. 分析参考图，提炼风格关键词，输出 `style_profile.json`
...
- 无参考图时跳过此步骤，`assemble_prompt.py` 使用中性回退（不带风格偏向的默认描述）。
...
脚本自动完成：
1. 读取 brief.json + distill_card.json + copywriting.json
2. 按规则程序化组装 prompt（参考图排序：风格→骨架→产品→IP→Logo）
3. 内置校验：检查无工作流术语、坐标覆盖率、文案覆盖率、参考图角色挂载是否冲突
...
是否清理本次任务的所有中间文件？
- 「是，清理」→ 删除项目目录下的所有中间文件，保留 final_poster.png
- 「否，保留」→ 保留所有文件
```
